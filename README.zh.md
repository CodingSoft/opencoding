...
brew install CodingSoft/tap/codingsoft # macOS 和 Linux（推荐，始终保持最新）
sudo pacman -S codingsoft            # Arch Linux (Stable)
paru -S codingsoft-bin               # Arch Linux (Latest from AUR)
mise use -g codingsoft               # 任意系统
nix run nixpkgs#codingsoft           # 或用 github:CodingSoft/codingsoft 获取最新 dev 分支
```
...
CodingSoft 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/CodingSoft/codingsoft/releases) 或 [codingsoft.vercel.app/download](https://codingsoft.vercel.app/download) 下载。
...
1. `$CODINGSOFT_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.codingsoft/bin` - 默认备用路径
...
CodingSoft 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://codingsoft.vercel.app/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://codingsoft.vercel.app/docs)。
...
如果你在项目名中使用了 “codingsoft”（如 “codingsoft-dashboard” 或 “codingsoft-mobile”），请在 README 里注明该项目不是 CodingSoft 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [CodingSoft Zen](https://opencoding.vercel.app/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
- 聚焦终端界面 (TUI)。CodingSoft 由 Neovim 爱好者和 [terminal.shop](https://terminal.shop) 的创建者打造，会持续探索终端的极限。
