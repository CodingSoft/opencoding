---
title: Servidor
description: Interaja com o servidor codingsoft via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `codingsoft serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente codingsoft pode usar.
...
```bash
codingsoft serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `codingsoft.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
codingsoft serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `CODINGSOFT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `codingsoft`, ou defina `CODINGSOFT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `codingsoft serve` quanto ao `codingsoft web`.

```bash
CODINGSOFT_SERVER_PASSWORD=your-password codingsoft serve
```
...
Quando você executa `codingsoft`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor codingsoft para interagir com o codingsoft programaticamente.
:::

Essa arquitetura permite que o codingsoft suporte múltiplos clientes e permite que você interaja com o codingsoft programaticamente.

Você pode executar `codingsoft serve` para iniciar um servidor autônomo. Se você tiver o TUI do codingsoft em execução, `codingsoft serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do codingsoft [IDE](/docs/ide).
...
O servidor codingsoft expõe as seguintes APIs.
