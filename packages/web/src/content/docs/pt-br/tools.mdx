...
As ferramentas permitem que o LLM execute ações em sua base de código. O codingsoft vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no codingsoft.
...
:::note
Esta ferramenta está disponível apenas quando `CODINGSOFT_EXPERIMENTAL_LSP_TOOL=true` (ou `CODINGSOFT_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor codingsoft ou quando a variável de ambiente `CODINGSOFT_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o codingsoft:
