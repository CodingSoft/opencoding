...
As habilidades do agente permitem que o codingsoft descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O codingsoft pesquisa nesses locais:

- Configuração do projeto: `.codingsoft/skills/<name>/SKILL.md`
- Configuração global: `~/.config/codingsoft/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o codingsoft sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.codingsoft/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/codingsoft/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.codingsoft/skills/git-release/SKILL.md` assim:
...
O codingsoft lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `codingsoft.json`:
...
**Para agentes embutidos** (em `codingsoft.json`):
