...
Com o codingsoft, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o codingsoft usa nosso próprio tema `codingsoft`.
...
O codingsoft vem com vários temas embutidos.
...
- Querem que o codingsoft corresponda à aparência do seu terminal
...
O codingsoft suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/codingsoft/themes/*.json` ou `$XDG_CONFIG_HOME/codingsoft/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.codingsoft/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.codingsoft/themes/*.json`
