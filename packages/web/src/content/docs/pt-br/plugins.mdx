---
title: Plugins
description: Escreva seus próprios plugins para estender o codingsoft.
---

Plugins permitem que você estenda o codingsoft conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do codingsoft.
...
- `.codingsoft/plugins/` - Plugins em nível de projeto
- `~/.config/codingsoft/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/codingsoft/node_modules/`.
...
1. Configuração global (`~/.config/codingsoft/codingsoft.json`)
2. Configuração do projeto (`codingsoft.json`)
3. Diretório de plugins global (`~/.config/codingsoft/plugins/`)
4. Diretório de plugins do projeto (`.codingsoft/plugins/`)
...
O codingsoft executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do codingsoft para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o codingsoft.
...
:::note
Se você estiver usando o aplicativo desktop codingsoft, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o codingsoft de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao codingsoft:
...
O helper `tool` cria uma ferramenta personalizada que o codingsoft pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o codingsoft junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://codingsoft.ai/docs/sdk) para detalhes.
