...
O CodingSoft Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

:::note
O CodingSoft Go está atualmente em beta.
:::

O Go funciona como qualquer outro provedor no CodingSoft. Você assina o CodingSoft Go e
...
O CodingSoft Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O CodingSoft Go funciona como qualquer outro provedor no CodingSoft.

1. Você entra no **<a href={console}>CodingSoft Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `CodingSoft Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o CodingSoft Go.
:::
...
O CodingSoft Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do CodingSoft
usa o formato `codingsoft-go/<model-id>`. Por exemplo, para o Kimi K2.5, você usaria
`codingsoft-go/kimi-k2.5` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o CodingSoft também.
