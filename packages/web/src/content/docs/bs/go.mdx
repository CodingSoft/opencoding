...
CodingSoft Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
CodingSoft Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u CodingSoft-u. Pretplatite se na CodingSoft Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili CodingSoft.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa CodingSoft-om.
:::
...
CodingSoft Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

CodingSoft Go radi kao bilo koji drugi provajder u CodingSoft-u.

1. Prijavite se na **<a href={console}>CodingSoft Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `CodingSoft Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na CodingSoft Go.
:::
...
CodingSoft Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj CodingSoft konfiguraciji
koristi format `codingsoft-go/<model-id>`. Na primjer, za Kimi K2.5, koristili biste
`codingsoft-go/kimi-k2.5` u svojoj konfiguraciji.
...
Napravili smo CodingSoft Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz CodingSoft koristite i bilo kojeg drugog provajdera.
