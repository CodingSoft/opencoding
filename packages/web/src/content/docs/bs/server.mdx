---
title: Server
description: Komunicirajte s codingsoft serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `codingsoft serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji codingsoft klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `codingsoft.local` |
...
Postavite `CODINGSOFT_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `codingsoft`, ili postavite `CODINGSOFT_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `codingsoft serve` i za `codingsoft web`.
...
Kada pokrenete `codingsoft`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite codingsoft server za programsku interakciju sa codingsoft-om.
:::

Ova arhitektura omogucava codingsoft podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `codingsoft serve` da startate standalone server. Ako je codingsoft TUI vec pokrenut, `codingsoft serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste CodingSoft [IDE](/docs/ide) pluginovi.
...
codingsoft server izlaže sljedece API-je.
