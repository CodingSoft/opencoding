...
Postoje dvije vrste agenata u CodingSoft: primarni agenti i podagenti.
...
CodingSoft dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
CodingSoft dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

CodingSoft dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `codingsoft.json`:
...
- Globalno: `~/.config/codingsoft/agents/`
- Po projektu: `.codingsoft/agents/`
...
Ako temperatura nije navedena, CodingSoft koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu CodingSoft konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj CodingSoft konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [CodingSoft Zen](/docs/zen), koristili biste `codingsoft/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `codingsoft models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/codingsoft).
:::
