...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. CodingSoft dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u CodingSoft.
...
:::note
Ovaj alat je dostupan samo kada je `CODINGSOFT_EXPERIMENTAL_LSP_TOOL=true` (ili `CODINGSOFT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz CodingSoft provajdera ili kada je varijabla `CODINGSOFT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju CodingSoft:
