---
title: Dodaci
description: Napišite vlastite dodatke za proširenje CodingSoft.
---

Dodaci vam omogućavaju da proširite CodingSoft spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja CodingSoft.
...
- `.codingsoft/plugins/` - Dodaci na nivou projekta
- `~/.config/codingsoft/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/codingsoft/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/codingsoft/codingsoft.json`)
2. Konfiguracija projekta (`codingsoft.json`)
3. Globalni direktorij dodataka (`~/.config/codingsoft/plugins/`)
4. Direktorij dodataka projekta (`.codingsoft/plugins/`)
...
CodingSoft pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje CodingSoft.
...
Spriječite codingsoft da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u codingsoft:
...
Pomoćnik `tool` kreira prilagođeni alat koji codingsoft može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za codingsoft zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://codingsoft.ai/docs/sdk) za detalje.
