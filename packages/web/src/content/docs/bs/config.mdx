---
title: Konfiguracija
description: Korištenje CodingSoft JSON konfiguracije.
---

Možete konfigurirati CodingSoft koristeći JSON konfiguracijski fajl.

---

## Format

CodingSoft podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/codingsoft`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/codingsoft/codingsoft.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CODINGSOFT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`codingsoft.json` u projektu) - postavke specifične za projekat
5. **`.codingsoft` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CODINGSOFT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.codingsoft` i `~/.config/codingsoft` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/codingsoft` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu CodingSoft konfiguraciju u `~/.config/codingsoft/codingsoft.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/codingsoft/tui.json`.
...
Dodajte `codingsoft.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se CodingSoft pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CODINGSOFT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CODINGSOFT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.codingsoft` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.codingsoft`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`codingsoft.ai/config.json`**](https://codingsoft.ai/config.json).

TUI konfiguracija koristi [**`codingsoft.ai/tui.json`**](https://codingsoft.ai/tui.json).
...
Koristite `CODINGSOFT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `codingsoft.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `codingsoft serve` i `codingsoft web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš CodingSoft server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `codingsoft.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj CodingSoft konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, CodingSoft pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/codingsoft/agents/` ili `.codingsoft/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, CodingSoft će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`codingsoft run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/codingsoft/commands/` ili `.codingsoft/commands/`. [Saznajte više ovdje](/docs/commands).
...
CodingSoft će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, CodingSoft **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju CodingSoft sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.codingsoft/plugins/` ili `~/.config/codingsoft/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite CodingSoft da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
