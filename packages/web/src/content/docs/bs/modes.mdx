...
ove ili konfigurirajte svoje putem codingsoft config.
...
codingsoft dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.codingsoft/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `codingsoft.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/codingsoft/modes/`
- Projekat: `.codingsoft/modes/`
...
Ako temperatura nije navedena, codingsoft koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.codingsoft/modes/` za specifične načine rada ili `~/.config/codingsoft/modes/` za globalne načine:
