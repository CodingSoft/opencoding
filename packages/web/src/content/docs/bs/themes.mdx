...
U CodingSoft mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, CodingSoft koristi nasu `codingsoft` temu.
...
CodingSoft dolazi sa vise ugradenih tema.
...
- Zele da CodingSoft odgovara izgledu njihovog terminala
...
CodingSoft podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/codingsoft/themes/*.json` ili `$XDG_CONFIG_HOME/codingsoft/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.codingsoft/themes/*.json`
4. **Trenutni radni direktorij** - `./.codingsoft/themes/*.json`
