---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u CodingSoft.
---

import config from "../../../../config.mjs"
export const console = config.console

CodingSoft koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj CodingSoft konfiguraciji.
...
u `~/.local/share/codingsoft/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem CodingSoft
config.
...
CodingSoft Zen je lista modela koje je obezbedio CodingSoft tim koji su bili
testirano i potvrđeno da dobro radi sa CodingSoft. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa CodingSoft Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite codingsoft i idite na [codingsoft.ai/auth](https://codingsoft.ai/auth).
...
Radi kao i svaki drugi provajder u CodingSoft i potpuno je opcionalan za korištenje.

---

## CodingSoft Go

CodingSoft Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža CodingSoft tim i koji su testirani i verificirani da dobro rade s CodingSoft-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `CodingSoft Go` i idite na [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
Da biste koristili Amazon Bedrock s CodingSoft:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete codingsoft:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `codingsoft.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u CodingSoft.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi codingsoft ispravno radio.
   :::
...
Također možete dodati modele kroz svoju codingsoft konfiguraciju.
...
da pohrani token u codingsoft auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
CodingSoft koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate CodingSoft
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`codingsoft.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [codingsoft-gitlab-auth](https://www.npmjs.com/package/codingsoft-gitlab-auth).

##### Konfiguracija

Prilagodite putem `codingsoft.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s codingsoft:
...
Za korištenje Google Vertex AI s CodingSoft:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće codingsoft.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem codingsofta, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [codingsoft-helicone-session](https://github.com/H2Shami/codingsoft-helicone-session) da automatski prijavite svaki CodingSoft razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki CodingSoft razgovor naveden kao zasebna sesija.
...
Možete konfigurirati codingsoft za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati codingsoft za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati codingsoft za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za CodingSoft. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/codingsoft) za detalje.
:::
...
Da biste koristili Ollama Cloud s CodingSoft:
...
3. Kopirajte API ključ za korištenje u CodingSoft.
...
6. **Važno**: Prije upotrebe modela oblaka u CodingSoft, morate lokalno povući informacije o modelu:
...
CodingSoft Zen je lista testiranih i verifikovanih modela koju je obezbedio CodingSoft tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>CodingSoft Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **CodingSoft Zen**.

```txt
   /connect
```

3. Unesite svoj CodingSoft API ključ.
...
Također možete dodati dodatne modele putem vaše codingsoft konfiguracije.
...
5. Također ih možete prilagoditi putem vaše codingsoft konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju codingsoft konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s codingsoft-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `codingsoft.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju CodingSoft da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `codingsoft auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite CodingSoft konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj codingsoft konfiguraciji.
