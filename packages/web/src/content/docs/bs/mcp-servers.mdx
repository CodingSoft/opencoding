...
Možete dodati vanjske alate u CodingSoft koristeći _Model Context Protocol_, ili MCP. CodingSoft podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [CodingSoft Config](https://codingsoft.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/codingsoft`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
CodingSoft automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, CodingSoft će:
...
Ako server zahtijeva autentifikaciju, CodingSoft će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `codingsoft mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, CodingSoft sigurno čuva tokene u `~/.local/share/codingsoft/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u CodingSoft, zajedno s ugrađenim alatima. Možete njima upravljati kroz CodingSoft konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete CodingSoft sa vašim Sentry nalogom.
