...
Agent skills omogucavaju CodingSoft da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
CodingSoft pretrazuje ove lokacije:

- Konfiguracija projekta: `.codingsoft/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/codingsoft/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, CodingSoft ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.codingsoft/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/codingsoft/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.codingsoft/skills/git-release/SKILL.md` ovako:
...
CodingSoft navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `codingsoft.json`:
...
**Za ugradene agente** (u `codingsoft.json`):
