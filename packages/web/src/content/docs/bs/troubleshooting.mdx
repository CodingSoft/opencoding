...
Da biste otklonili probleme s CodingSoft, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codingsoft\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `codingsoft --log-level DEBUG`.

---

## Pohrana

CodingSoft pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/codingsoft/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codingsoft`
...
CodingSoft Desktop pokreće lokalni CodingSoft server (`codingsoft-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `CodingSoft` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/codingsoft/codingsoft.jsonc` (ili `~/.config/codingsoft/codingsoft.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/codingsoft/codingsoft.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codingsoft\codingsoft.jsonc`
...
CodingSoft također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/codingsoft/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codingsoft\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga CodingSoft mogao ponovo izgraditi.

1. Potpuno zatvorite CodingSoft Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/codingsoft`
- **Linux**: obrišite `~/.cache/codingsoft` (ili pokrenite `rm -rf ~/.cache/codingsoft`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\codingsoft`

3. Ponovo pokrenite CodingSoft Desktop.

---

### Rješavanje problema sa vezom na serveru

CodingSoft Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `codingsoft.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CODINGSOFT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CODINGSOFT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, CodingSoft Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa CodingSoft karakteristikama.

---

### Obavještenja se ne prikazuju

CodingSoft Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za CodingSoft u postavkama vašeg OS-a, i
...
1. Zatvorite CodingSoft Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka CodingSoft Desktop aplikacije):

- `codingsoft.settings.dat` (URL zadanog servera za desktop)
- `codingsoft.global.dat` i `codingsoft.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s CodingSoft:
...
Da saznate kojim modelima imate pristup, pokrenite `codingsoft models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\codingsoft`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. CodingSoft dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\codingsoft`

2. Ponovo pokrenite CodingSoft da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti CodingSoft da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
CodingSoft će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
