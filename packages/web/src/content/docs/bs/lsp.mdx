---
title: LSP serveri
description: CodingSoft se integriše sa vašim LSP serverima.
---

CodingSoft se integriše sa vašim Language Server Protocol (LSP) serverima kako bi pomogao LLM-u u interakciji s kodnom bazom. Koristi dijagnostiku za slanje povratnih informacija modelu.

## Ugrađeni

CodingSoft dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `CODINGSOFT_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada codingsoft otvori fajl, on:
...
Možete prilagoditi LSP servere kroz `lsp` odjeljak u vašoj codingsoft konfiguraciji.
