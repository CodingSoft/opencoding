---
title: SDK
description: Client JS de type sécurisé pour le serveur codingsoft.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK codingsoft JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler codingsoft par programme.
...
Créez une instance de codingsoft :

```javascript
import { createOpencode } from "@codingsoft-ai/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `codingsoft.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${codingsoft.server.url}`)

codingsoft.server.close()
```

## Client uniquement

Si vous disposez déjà d'une instance en cours d'exécution de codingsoft, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createOpencodeClient } from "@codingsoft-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@codingsoft-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*codingsoft" },
})
