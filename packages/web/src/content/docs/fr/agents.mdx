...
Il existe deux types d'agents dans CodingSoft ; agents primaires et sous-agents.
...
CodingSoft est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
CodingSoft est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

CodingSoft est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `codingsoft.json` :
...
- Global : `~/.config/codingsoft/agents/`
- Par projet : `.codingsoft/agents/`

```markdown title="~/.config/codingsoft/agents/review.md"
---
...
Si aucune température n'est spécifiée, CodingSoft utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale CodingSoft et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration CodingSoft utilise le format `provider/model-id`. Par exemple, si vous utilisez [CodingSoft Zen](/docs/zen), vous utiliserez `codingsoft/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="codingsoft.json" {3-6,9-12}
{
  "$schema": "https://codingsoft.ai/config.json",
  "tools": {
    "write": true,
...
```json title="codingsoft.json" {3-5,8-10}
{
  "$schema": "https://codingsoft.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="codingsoft.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Exécutez `codingsoft models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/codingsoft).
:::
...
```markdown title="~/.config/codingsoft/agents/docs-writer.md"
---
...
```markdown title="~/.config/codingsoft/agents/security-auditor.md"
---
