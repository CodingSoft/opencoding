---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en CodingSoft.
---

import config from "../../../../config.mjs"
export const console = config.console

CodingSoft utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration CodingSoft.
...
en `~/.local/share/codingsoft/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre CodingSoft
configuration.
...
CodingSoft Zen est une liste de modèles fournis par l'équipe CodingSoft qui ont été
testé et vérifié pour fonctionner correctement avec CodingSoft. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par CodingSoft Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez codingsoft et dirigez-vous vers [codingsoft.ai/auth](https://codingsoft.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans CodingSoft et son utilisation est totalement facultative.

---

## CodingSoft Go

CodingSoft Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe CodingSoft qui ont été
testé et vérifié pour fonctionner correctement avec CodingSoft.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `CodingSoft Go` et rendez-vous sur [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
Pour utiliser Amazon Bedrock avec CodingSoft :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de codingsoft :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY codingsoft

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile codingsoft

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX codingsoft
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `codingsoft.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans CodingSoft n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que codingsoft fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX codingsoft
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX codingsoft
   ```
...
Vous pouvez également ajouter des modèles via votre configuration codingsoft.
...
pour stocker le jeton dans le stockage d'authentification codingsoft.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
CodingSoft utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller CodingSoft
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `codingsoft.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [codingsoft-gitlab-auth](https://www.npmjs.com/package/codingsoft-gitlab-auth).

##### Configuration

Personnalisez via `codingsoft.json` :
...
{
  "$schema": "https://codingsoft.ai/config.json",
  "plugin": ["codingsoft-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec codingsoft :
...
Pour utiliser Google Vertex AI avec CodingSoft :
...
Définissez-les lors de l'exécution de codingsoft.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id codingsoft
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via codingsoft, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/codingsoft/codingsoft.jsonc"
{
  "$schema": "https://codingsoft.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [codingsoft-helicone-session](https://github.com/H2Shami/codingsoft-helicone-session) pour enregistrer automatiquement chaque conversation CodingSoft en tant que session dans Helicone.

```bash
npm install -g codingsoft-helicone-session
```
...
```json title="codingsoft.json"
{
  "plugin": ["codingsoft-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation CodingSoft répertoriée comme une session distincte.
...
Vous pouvez configurer codingsoft pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="codingsoft.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://codingsoft.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer codingsoft pour utiliser des modèles locaux via LM Studio.

```json title="codingsoft.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://codingsoft.ai/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer codingsoft pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour CodingSoft. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/codingsoft) pour plus de détails.
:::

```json title="codingsoft.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://codingsoft.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec CodingSoft :
...
3. Copiez la clé API à utiliser dans CodingSoft.
...
6. **Important** : Avant d'utiliser des modèles cloud dans CodingSoft, vous devez extraire les informations du modèle localement :
...
CodingSoft Zen est une liste de modèles testés et vérifiés fournie par l'équipe CodingSoft. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>CodingSoft Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **CodingSoft Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé CodingSoft API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration codingsoft.
...
5. Vous pouvez également les personnaliser via votre configuration codingsoft. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' codingsoft
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group codingsoft
   ```
...
Vous pouvez également personnaliser les modèles via votre configuration codingsoft. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec codingsoft. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in codingsoft.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `codingsoft.json` dans le répertoire de votre projet :

   ```json title="codingsoft.json" ""myprovider"" {5-15}
   {
     "$schema": "https://codingsoft.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="codingsoft.json" {9,11,17-20}
{
  "$schema": "https://codingsoft.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à CodingSoft de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `codingsoft auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration codingsoft et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration codingsoft.
