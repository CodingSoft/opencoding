---
title: TUI
description: Utilisation de l’interface utilisateur du terminal CodingSoft.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodingSoft fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de CodingSoft démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le CodingSoft TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à CodingSoft. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez CodingSoft. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `codingsoft.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `CODINGSOFT_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.
