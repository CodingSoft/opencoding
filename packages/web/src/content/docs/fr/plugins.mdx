---
title: Plugins
description: Écrivez vos propres plugins pour étendre CodingSoft.
---

Les plugins vous permettent d'étendre CodingSoft en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de CodingSoft.
...
- `.codingsoft/plugins/` - Plugins au niveau du projet
- `~/.config/codingsoft/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/codingsoft/node_modules/`.
...
1. Configuration globale (`~/.config/codingsoft/codingsoft.json`)
2. Configuration du projet (`codingsoft.json`)
3. Répertoire global des plugins (`~/.config/codingsoft/plugins/`)
4. Répertoire des plugins du projet (`.codingsoft/plugins/`)
...
CodingSoft exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK codingsoft pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre codingsoft.
...
:::note
Si vous utilisez l'application de bureau CodingSoft, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez codingsoft de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à codingsoft :
...
L'assistant `tool` crée un outil personnalisé que codingsoft peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour codingsoft aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://codingsoft.ai/docs/sdk) pour plus de détails.
