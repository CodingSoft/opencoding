...
Avec CodingSoft, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, CodingSoft utilise notre propre thème `codingsoft`.
...
CodingSoft est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que CodingSoft corresponde à l'apparence de leur terminal
...
CodingSoft prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/codingsoft/themes/*.json` ou `$XDG_CONFIG_HOME/codingsoft/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.codingsoft/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.codingsoft/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/codingsoft/themes
vim ~/.config/codingsoft/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .codingsoft/themes
vim .codingsoft/themes/my-theme.json
```
...
{
  "$schema": "https://codingsoft.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
