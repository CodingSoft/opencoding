...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez CodingSoft.
...
Voici plusieurs modèles qui fonctionnent bien avec CodingSoft, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration CodingSoft.

```json title="codingsoft.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [CodingSoft Zen](/docs/zen), vous utiliserez `codingsoft/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="codingsoft.jsonc" {7-12,19-24}
{
  "$schema": "https://codingsoft.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="codingsoft.jsonc" {6-21}
{
  "$schema": "https://codingsoft.ai/config.json",
  "provider": {
    "codingsoft": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. CodingSoft est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

CodingSoft est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="codingsoft.jsonc" {7-18}
{
  "$schema": "https://codingsoft.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque CodingSoft démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration CodingSoft.
