---
title: Serveur
description: Interagissez avec le serveur codingsoft via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `codingsoft serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client codingsoft peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `codingsoft.local` |
...
Définissez `CODINGSOFT_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `codingsoft`, ou définissez `CODINGSOFT_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `codingsoft serve` et à `codingsoft web`.
...
Lorsque vous exécutez `codingsoft`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur codingsoft pour interagir avec codingsoft par programmation.
:::

Cette architecture permet à codingsoft de prendre en charge plusieurs clients et vous permet d'interagir avec codingsoft par programme.

Vous pouvez exécuter `codingsoft serve` pour démarrer un serveur autonome. Si vous avez le
codingsoft TUI en cours d'exécution, `codingsoft serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins CodingSoft [IDE](/docs/ide).
...
Le serveur codingsoft expose les API suivantes.
