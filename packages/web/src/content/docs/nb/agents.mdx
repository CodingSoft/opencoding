...
Det er to typer agenter i CodingSoft; primære agenter og underagenter.
...
CodingSoft kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
CodingSoft kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

CodingSoft kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `codingsoft.json`:
...
- Globalt: `~/.config/codingsoft/agents/`
- Per prosjekt: `.codingsoft/agents/`
...
Hvis ingen temperatur er spesifisert, bruker CodingSoft modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale CodingSoft-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i CodingSoft-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [CodingSoft Zen](/docs/zen), vil du bruke `codingsoft/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `codingsoft models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/codingsoft).
:::
