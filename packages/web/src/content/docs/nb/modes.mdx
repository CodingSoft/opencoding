...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i CodingSoft-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i CodingSoft lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom CodingSoft-konfigurasjonen.
...
CodingSoft kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.codingsoft/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `codingsoft.json` konfigurasjonsfil:
...
- Globalt: `~/.config/codingsoft/modes/`
- Prosjekt: `.codingsoft/modes/`
...
Hvis ingen temperatur er spesifisert, bruker CodingSoft modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale CodingSoft-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.codingsoft/modes/` for prosjektspesifikke moduser eller `~/.config/codingsoft/modes/` for globale moduser:
