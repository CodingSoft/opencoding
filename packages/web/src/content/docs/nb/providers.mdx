---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i CodingSoft.
---

import config from "../../../../config.mjs"
export const console = config.console

CodingSoft bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i CodingSoft-konfigurasjonen.
...
i `~/.local/share/codingsoft/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i CodingSoft
konfig.
...
CodingSoft Zen er en liste over modeller levert av CodingSoft-teamet som har vært
testet og verifisert for å fungere godt med CodingSoft. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med CodingSoft Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `CodingSoft Zen` og gå til [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
Det fungerer som alle andre leverandører i CodingSoft og er helt valgfritt å bruke.

---

## CodingSoft Go

CodingSoft Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av CodingSoft-teamet som har vært
testet og verifisert for å fungere godt med CodingSoft.

1. Kjør kommandoen `/connect` i TUI, velg `CodingSoft Go`, og gå til [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
Slik bruker du Amazon Bedrock med CodingSoft:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører CodingSoft:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `codingsoft.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i codingsoft støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at CodingSoft skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom CodingSoft-konfigurasjonen.
...
for å lagre token i CodingSoft auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
CodingSoft bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse CodingSoft
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`codingsoft.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [codingsoft-gitlab-auth](https://www.npmjs.com/package/codingsoft-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `codingsoft.json`:
...
Slik bruker du GitHub Copilot-abonnementet med CodingSoft:
...
Slik bruker du Google Vertex AI med CodingSoft:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører CodingSoft.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom CodingSoft, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [codingsoft-helicone-session](https://github.com/H2Shami/codingsoft-helicone-session) plugin for automatisk å logge hver CodingSoft-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver CodingSoft-samtale oppført som en separat økt.
...
Du kan konfigurere CodingSoft for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere CodingSoft for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere CodingSoft for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for CodingSoft. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/codingsoft) for detaljer.
:::
...
Slik bruker du Ollama Cloud med CodingSoft:
...
3. Kopier API-nøkkelen for bruk i CodingSoft.
...
6. **Viktig**: Før du bruker skymodeller i CodingSoft, må du hente modellinformasjonen lokalt:
...
CodingSoft Zen er en liste over testede og verifiserte modeller levert av CodingSoft-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>CodingSoft Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **CodingSoft Zen**.

   ```txt
   /connect
   ```

3. Skriv inn CodingSoft API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom CodingSoft-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom CodingSoft-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CodingSoft:
...
Du kan også tilpasse modeller gjennom CodingSoft-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med CodingSoft. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `codingsoft.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar CodingSoft forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `codingsoft auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk CodingSoft-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i CodingSoft-konfigurasjonen.
