...
Agentferdigheter lar CodingSoft oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
codingsoft søker etter disse stedene:

- Prosjektkonfigurasjon: `.codingsoft/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/codingsoft/skills/<name>/SKILL.md`
...
For prosjektlokale stier går codingsoft opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.codingsoft/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/codingsoft/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.codingsoft/skills/git-release/SKILL.md` slik:
...
codingsoft viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `codingsoft.json`:
...
**For innebygde agenter** (i `codingsoft.json`):
