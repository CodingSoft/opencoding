...
description: Skriv dine egne programtillegg for å utvide CodingSoft.
---

Plugins lar deg utvide CodingSoft ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til CodingSoft.
...
- `.codingsoft/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/codingsoft/node_modules/`.
...
1. Global konfigurasjon (`~/.config/codingsoft/codingsoft.json`)
2. Prosjektkonfigurasjon (`codingsoft.json`)
3. Global plugin-katalog (`~/.config/codingsoft/plugins/`)
4. Prosjektpluginkatalog (`.codingsoft/plugins/`)
...
CodingSoft kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En CodingSoft SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide CodingSoft.
...
:::note
Hvis du bruker CodingSoft-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre CodingSoft fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til CodingSoft:
...
`tool`-hjelperen lager et tilpasset verktøy som CodingSoft kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for CodingSoft sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://codingsoft.ai/docs/sdk) for detaljer.
