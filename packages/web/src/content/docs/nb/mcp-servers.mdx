...
Du kan legge til eksterne verktøy i CodingSoft ved å bruke _Model Context Protocol_ eller MCP. CodingSoft støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [CodingSoft Config](https://codingsoft.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="codingsoft.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/codingsoft`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="codingsoft.jsonc" {15}
{
  "$schema": "https://codingsoft.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
CodingSoft håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil CodingSoft:
...
Hvis serveren krever autentisering, vil CodingSoft be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `codingsoft mcp auth <server-name>`.
...
```json title="codingsoft.json" {7-11}
{
  "$schema": "https://codingsoft.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
codingsoft mcp auth my-oauth-server
```
...
```bash
codingsoft mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil CodingSoft lagre tokens sikkert i `~/.local/share/codingsoft/mcp-auth.json`.
...
codingsoft mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i CodingSoft, sammen med innebygde verktøy. Så du kan administrere dem gjennom CodingSoft-konfigurasjonen som alle andre verktøy.
...
```json title="codingsoft.json" {11, 14-18}
{
  "$schema": "https://codingsoft.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="codingsoft.json" {4-8}
{
  "$schema": "https://codingsoft.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
codingsoft mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble CodingSoft til Sentry-kontoen din.
...
```json title="codingsoft.json" {4-7}
{
  "$schema": "https://codingsoft.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="codingsoft.json" {7-9}
{
  "$schema": "https://codingsoft.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver CodingSoft-samtale oppført som en separat økt.
