---
title: Tjener
description: Samhandle med CodingSoft-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codingsoft serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en CodingSoft-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `codingsoft.local` |
...
Sett `CODINGSOFT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `codingsoft` som standard, eller sett `CODINGSOFT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `codingsoft serve` og `codingsoft web`.
...
Når du kjører `codingsoft` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk CodingSoft-serveren til å samhandle med CodingSoft programmatisk.
:::

Denne arkitekturen lar CodingSoft støtte flere klienter og lar deg samhandle med CodingSoft programmatisk.

Du kan kjøre `codingsoft serve` for å starte en frittstående server. Hvis du har
CodingSoft TUI kjører, vil `codingsoft serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av CodingSoft [IDE](/docs/ide) plugins.
...
codingsoft-serveren viser følgende APIer.
