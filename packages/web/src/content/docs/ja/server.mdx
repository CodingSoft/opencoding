---
title: サーバー
description: HTTP 経由で CodingSoft サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codingsoft serve` コマンドは、CodingSoft クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `codingsoft.local` |
...
HTTP 基本認証でサーバーを保護するには、`CODINGSOFT_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `codingsoft` になるか、`CODINGSOFT_SERVER_USERNAME` を設定してオーバーライドします。これは、`codingsoft serve` と `codingsoft web` の両方に当てはまります。
...
`codingsoft` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
CodingSoft サーバーを使用して、プログラムで CodingSoft と対話します。
:::

`codingsoft serve` を実行してスタンドアロンサーバーを起動できます。すでに CodingSoft TUI を実行している場合でも、`codingsoft serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、CodingSoft [IDE](/docs/ide) プラグインによって使用されます。
