...
Навыки агента позволяют codingsoft обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
codingsoft выполняет поиск в следующих местах:

- Конфигурация проекта: `.codingsoft/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/codingsoft/skills/<name>/SKILL.md`.
...
Для локальных путей проекта codingsoft переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.codingsoft/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/codingsoft/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.codingsoft/skills/git-release/SKILL.md` следующим образом:
...
codingsoft перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `codingsoft.json`:
...
**Для встроенных агентов** (в формате `codingsoft.json`):
