...
Создайте `.codingsoft/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию codingsoft или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем codingsoft [config](/docs/config):
...
- Глобальный: `~/.config/codingsoft/commands/`
- Для каждого проекта: `.codingsoft/commands/`
...
codingsoft включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
