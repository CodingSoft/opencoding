---
title: Plugins
description: Skriv dine egne plugins for at udvide CodingSoft.
---

Plugins giver dig mulighed for at udvide CodingSoft ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre CodingSofts standardadfærd.
...
- `.codingsoft/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/codingsoft/node_modules/`.
...
1. Global konfiguration (`~/.config/codingsoft/codingsoft.json`)
2. Projektkonfiguration (`codingsoft.json`)
3. Global plugin-mappe (`~/.config/codingsoft/plugins/`)
4. Projekt plugin bibliotek (`.codingsoft/plugins/`)
...
CodingSoft kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En codingsoft SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide codingsoft.
...
:::note
Hvis du bruger CodingSoft desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå codingsoft i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til codingsoft:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som codingsoft kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for codingsoft sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://codingsoft.ai/docs/sdk) for detaljer.
