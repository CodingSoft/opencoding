---
title: Server
description: Interager med codingsoft-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codingsoft serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en codingsoft-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `codingsoft.local` |
...
Indstil `CODINGSOFT_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `codingsoft`, eller indstil `CODINGSOFT_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `codingsoft serve` og `codingsoft web`.
...
Når du kører `codingsoft` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug codingsoft-serveren til at interagere med codingsoft programmatisk.
:::

Denne arkitektur lader codingsoft understøtte flere klienter og lader dig interagere med codingsoft programmatisk.

Du kan køre `codingsoft serve` for at starte en selvstændig server. Hvis du har
codingsoft TUI kørende, vil `codingsoft serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af CodingSoft [IDE](/docs/ide) plugins.
...
CodingSoft-serveren viser følgende API'er.
