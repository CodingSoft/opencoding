---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i CodingSoft.
---

import config from "../../../../config.mjs"
export const console = config.console

CodingSoft bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i CodingSoft-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/codingsoft/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i CodingSoft-konfigurationen.
...
CodingSoft Zen er en liste over modeller leveret af CodingSoft-teamet, som er blevet testet og verificeret til at fungere godt med CodingSoft. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med CodingSoft Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg codingsoft og gå til [codingsoft.ai/auth](https://codingsoft.ai/auth).
...
Det fungerer som alle andre udbydere i CodingSoft og er helt valgfrit at bruge.

---

## CodingSoft Go

CodingSoft Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af CodingSoft-teamet, som er testet og verificeret til at fungere godt med CodingSoft.

1. Kør kommandoen `/connect` i TUI, vælg `CodingSoft Go`, og gå til [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
Sådan bruger du Amazon Bedrock med CodingSoft:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører codingsoft:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `codingsoft.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i CodingSoft understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at codingsoft skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem codingsoft-konfigurationen.
...
gemme tokenet i codingsoft auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
CodingSoft bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse CodingSoft
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`codingsoft.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [codingsoft-gitlab-auth](https://www.npmjs.com/package/codingsoft-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `codingsoft.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med CodingSoft:
...
Sådan bruger du Google Vertex AI med CodingSoft:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører codingsoft.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem codingsoft, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [codingsoft-helicone-session](https://github.com/H2Shami/codingsoft-helicone-session) plugin for automatisk at logge hver CodingSoft-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver CodingSoft-samtale opført som en separat session.
...
Du kan konfigurere codingsoft til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere codingsoft til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere codingsoft til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for CodingSoft. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/codingsoft) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med CodingSoft:
...
3. Kopier API-nøglen til brug i CodingSoft.
...
6. **Vigtigt**: Før du bruger skymodeller i CodingSoft, skal du hente modelinformationen lokalt:
...
CodingSoft Zen er en liste over testede og verificerede modeller leveret af CodingSoft-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>CodingSoft Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **CodingSoft Zen**.

   ```txt
   /connect
   ```

3. Indtast CodingSoft API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem codingsoft-konfigurationen.
...
5. Du kan også tilpasse dem gennem codingsoft-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CodingSoft:
...
Du kan også tilpasse modeller gennem codingsoft-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med codingsoft. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `codingsoft.json`-filen i projektkataloget:
...
`limit`-felterne lader CodingSoft forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `codingsoft auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek codingsoft-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i codingsoft-konfigurationen.
