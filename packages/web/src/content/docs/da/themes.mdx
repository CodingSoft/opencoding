...
Med CodingSoft kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger CodingSoft vårt eget `codingsoft`-tema.
...
CodingSoft kommer med flere innebygde temaer.
...
- Vil at CodingSoft skal matche terminalens utseende
...
CodingSoft støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/codingsoft/themes/*.json` eller `$XDG_CONFIG_HOME/codingsoft/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.codingsoft/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.codingsoft/themes/*.json`
