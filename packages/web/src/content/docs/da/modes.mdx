...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i codingsoft-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i codingsoft giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem codingsoft-konfigurationen.
...
codingsoft leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.codingsoft/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `codingsoft.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger codingsoft modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale codingsoft-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.codingsoft/modes/` for projektspecifikke tilstande eller `~/.config/codingsoft/modes/` for globale tilstande:
