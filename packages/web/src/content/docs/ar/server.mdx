---
title: الخادم
description: تفاعل مع خادم codingsoft عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `codingsoft serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل codingsoft استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `codingsoft.local` |
...
عيّن `CODINGSOFT_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `codingsoft`، أو عيّن `CODINGSOFT_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `codingsoft serve` و `codingsoft web`.
...
عند تشغيل `codingsoft` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم codingsoft للتفاعل مع codingsoft برمجيا.
:::

تتيح هذه البنية لـ codingsoft دعم عدة عملاء وتمكّنك من التفاعل مع codingsoft برمجيا.

يمكنك تشغيل `codingsoft serve` لبدء خادم مستقل. إذا كانت واجهة codingsoft في terminal (TUI)
قيد التشغيل، فسيبدأ `codingsoft serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات CodingSoft لـ [IDE](/docs/ide).
...
يعرض خادم codingsoft واجهات API التالية.
