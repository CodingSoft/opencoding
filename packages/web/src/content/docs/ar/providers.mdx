---
title: المزوّدون
description: استخدام أي موفّر LLM في CodingSoft.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم CodingSoft ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات CodingSoft.
...
في `~/.local/share/codingsoft/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات CodingSoft.
...
CodingSoft Zen هي قائمة نماذج يوفّرها فريق CodingSoft وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع CodingSoft. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع CodingSoft Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر codingsoft، ثم انتقل إلى [codingsoft.ai/auth](https://codingsoft.ai/auth).
...
يعمل مثل أي مزوّد آخر في CodingSoft واستخدامه اختياري بالكامل.

---

## CodingSoft Go

CodingSoft Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق CodingSoft، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع CodingSoft.

1. شغّل الأمر `/connect` في TUI، واختر `CodingSoft Go`، ثم انتقل إلى [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
لاستخدام Amazon Bedrock مع CodingSoft:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل codingsoft:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `codingsoft.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل CodingSoft غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل codingsoft بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات codingsoft.
...
تخزين الرمز في مساحة تخزين مصادقة codingsoft.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم CodingSoft نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر CodingSoft
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`codingsoft.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [codingsoft-gitlab-auth](https://www.npmjs.com/package/codingsoft-gitlab-auth).

##### التهيئة

خصّص عبر `codingsoft.json`:
...
لاستخدام اشتراك GitHub Copilot مع codingsoft:
...
لاستخدام Google Vertex AI مع CodingSoft:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر codingsoft، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [codingsoft-helicone-session](https://github.com/H2Shami/codingsoft-helicone-session) لتسجيل كل محادثة CodingSoft تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة CodingSoft مدرجة كجلسة مستقلة.
...
يمكنك ضبط codingsoft لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط codingsoft لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط codingsoft لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع CodingSoft. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/codingsoft) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع CodingSoft:
...
3. انسخ مفتاح API لاستخدامه في CodingSoft.
...
6. **مهم**: قبل استخدام نماذج السحابة في CodingSoft، يجب سحب معلومات النموذج محليا:
...
CodingSoft Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق CodingSoft. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>CodingSoft Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **CodingSoft Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ CodingSoft.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات codingsoft.
...
5. يمكنك أيضا تخصيصها عبر إعدادات codingsoft. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات codingsoft. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع codingsoft. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `codingsoft.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ CodingSoft بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `codingsoft auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات codingsoft وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات codingsoft.
