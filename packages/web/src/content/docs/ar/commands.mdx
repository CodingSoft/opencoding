...
أنشئ الملف `.codingsoft/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات CodingSoft أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ CodingSoft:
...
- على مستوى النظام: `~/.config/codingsoft/commands/`
- لكل مشروع: `.codingsoft/commands/`
...
يتضمن codingsoft عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
