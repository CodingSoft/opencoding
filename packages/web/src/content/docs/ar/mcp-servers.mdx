...
يمكنك إضافة أدوات خارجية إلى CodingSoft باستخدام _Model Context Protocol_، أو MCP. يدعم CodingSoft خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات CodingSoft](https://codingsoft.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/codingsoft` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى CodingSoft تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم CodingSoft بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك CodingSoft المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `codingsoft mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن CodingSoft الرموز بشكل آمن في `~/.local/share/codingsoft/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل CodingSoft إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات CodingSoft مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط CodingSoft بحساب Sentry الخاص بك.
