...
:::caution
Modes are now configured through the `agent` option in the codingsoft config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in codingsoft allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the codingsoft config.
...
codingsoft comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.codingsoft/plans/*.md` to detail the plan itself
...
Configure modes in your `codingsoft.json` config file:
...
both the global codingsoft config and the project specific config.
...
Create mode files in `.codingsoft/modes/` for project-specific modes or `~/.config/codingsoft/modes/` for global modes:
