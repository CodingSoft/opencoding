---
title: TUI
description: CodingSoft terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodingSoft, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

**CodingSoft**'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
CodingSoft TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
CodingSoft'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
CodingSoft'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `codingsoft.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `CODINGSOFT_TUI_CONFIG` kullanın.
