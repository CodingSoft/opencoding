...
CodingSoft Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
CodingSoft Go şu anda beta olarak mevcut.
:::

Go, CodingSoft'daki diğer sağlayıcılar gibi çalışır. CodingSoft Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve CodingSoft'u kullanmak için bunu kullanmanıza gerek yoktur.
...
CodingSoft Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

CodingSoft Go, CodingSoft'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>CodingSoft Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `CodingSoft Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye CodingSoft Go'ya abone olabilir.
:::
...
CodingSoft Go aşağıdaki limitleri içerir:
...
CodingSoft yapılandırmanızdaki [model id](/docs/config/#models) formatı `codingsoft-go/<model-id>` şeklindedir. Örneğin, Kimi K2.5 için yapılandırmanızda `codingsoft-go/kimi-k2.5` kullanmalısınız.
...
CodingSoft Go'yu şu amaçlarla oluşturduk:
...
4. CodingSoft ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
