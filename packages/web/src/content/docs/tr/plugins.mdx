---
title: Eklentiler
description: CodingSoft'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek CodingSoft'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya CodingSoft'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.codingsoft/plugins/` - Proje düzeyinde eklentiler
- `~/.config/codingsoft/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/codingsoft/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/codingsoft/codingsoft.json`)
2. Project config (`codingsoft.json`)
3. Global eklenti dizini (`~/.config/codingsoft/plugins/`)
4. Proje eklenti dizini (`.codingsoft/plugins/`)
...
CodingSoft bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir CodingSoft SDK istemcisi.
...
CodingSoft'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
CodingSoft masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
CodingSoft'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca CodingSoft'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, CodingSoft'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra CodingSoft'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://codingsoft.ai/docs/sdk) bakın.
