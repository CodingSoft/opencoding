...
_Model Bağlam Protokolü_ veya MCP kullanarak codingsoft'a harici araçlar ekleyebilirsiniz. codingsoft hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [codingsoft Config](https://codingsoft.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/codingsoft` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
codingsoft, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde codingsoft şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa codingsoft, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `codingsoft mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından codingsoft, belirteçleri `~/.local/share/codingsoft/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra codingsoft'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi codingsoft yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve codingsoft'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
