...
:::caution
Modlar artık codingsoft yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

codingsoft'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya codingsoft yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
codingsoft iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.codingsoft/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`codingsoft.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, codingsoft modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global codingsoft yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.codingsoft/modes/` veya genel modlar için `~/.config/codingsoft/modes/`'de mod dosyaları oluşturun:
