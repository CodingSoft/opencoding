---
title: Sağlayıcılar
description: codingsoft'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

codingsoft, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı codingsoft yapılandırmanızda yapılandırın.
...
`~/.local/share/codingsoft/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları codingsoft'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
CodingSoft Zen, codingsoft ekibi tarafından sağlanan modellerin bir listesidir.
codingsoft ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz CodingSoft Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, codingsoft'u seçin ve [codingsoft.ai/auth](https://codingsoft.ai/auth)'ye gidin.
...
codingsoft'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## CodingSoft Go

CodingSoft Go, codingsoft ile iyi çalıştığı test edilmiş ve doğrulanmış, codingsoft ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `CodingSoft Go`'yu seçin ve [codingsoft.ai/auth](https://codingsoft.ai/zen) adresine gidin.
...
Amazon Bedrock'u codingsoft ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   codingsoft'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `codingsoft.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi codingsoft'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   codingsoft'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in codingsoft auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
codingsoft, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. codingsoft'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`codingsoft.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [codingsoft-gitlab-auth](https://www.npmjs.com/package/codingsoft-gitlab-auth) ana sayfasında.

##### Yapılandırma

`codingsoft.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi codingsoft'la kullanmak için:
...
Google Vertex AI'yi codingsoft ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları codingsoft çalıştırılırken ayarlayın.
...
Helicone'un codingsoft aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her codingsoft görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [codingsoft-helicone-session](https://github.com/H2Shami/codingsoft-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her codingsoft görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
codingsoft'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla codingsoft'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla codingsoft'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini codingsoft için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/codingsoft)'a bakın.
:::
...
Ollama Cloud'u codingsoft ile kullanmak için:
...
3. codingsoft'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: codingsoft'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
CodingSoft Zen, codingsoft ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>CodingSoft Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **CodingSoft Zen**'i arayın.

   ```txt
   /connect
   ```

3. codingsoft API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   codingsoft yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca codingsoft yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri codingsoft yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı codingsoft ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `codingsoft.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları codingsoft'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `codingsoft auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için codingsoft yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, codingsoft yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
