...
codingsoft ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak codingsoft kendi `codingsoft` temasını kullanır.
...
codingsoft birden fazla yerleşik temayla gelir.
...
- codingsoft'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
codingsoft, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/codingsoft/themes/*.json` veya `$XDG_CONFIG_HOME/codingsoft/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.codingsoft/themes/*.json`
4. **Geçerli çalışma dizini** - `./.codingsoft/themes/*.json`
