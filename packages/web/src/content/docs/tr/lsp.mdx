...
description: CodingSoft, LSP sunucularınızla bütünleşir.
---

CodingSoft, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

CodingSoft, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`CODINGSOFT_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
codingsoft bir dosyayı açtığında:
...
LSP sunucularını, codingsoft yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
