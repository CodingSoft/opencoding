---
title: Proveedores
description: Usando cualquier proveedor LLM en CodingSoft.
---

import config from "../../../../config.mjs"
export const console = config.console

CodingSoft usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración CodingSoft.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu CodingSoft
configuración.
...
CodingSoft Zen es una lista de modelos proporcionados por el equipo CodingSoft que han sido
probado y verificado para funcionar bien con CodingSoft. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con CodingSoft Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione codingsoft y diríjase a [codingsoft.ai/auth](https://codingsoft.ai/auth).
...
Funciona como cualquier otro proveedor en CodingSoft y su uso es completamente opcional.

---

## CodingSoft Go

CodingSoft Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de CodingSoft que han sido
probado y verificado para funcionar bien con CodingSoft.

1. Ejecute el comando `/connect` en TUI, seleccione `CodingSoft Go` y diríjase a [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
Para usar Amazon Bedrock con CodingSoft:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta codingsoft:
   ...
   Para una configuración persistente o específica del proyecto, utilice `codingsoft.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en CodingSoft no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que codingsoft funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración codingsoft.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `CodingSoft`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación codingsoft.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
CodingSoft utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear CodingSoft
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `codingsoft.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [codingsoft-gitlab-auth](https://www.npmjs.com/package/codingsoft-gitlab-auth).

##### Configuración

Personalizar a través de `codingsoft.json`:
...
Para utilizar su suscripción GitHub Copilot con codingsoft:
...
Para utilizar Google Vertex AI con CodingSoft:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta codingsoft.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de codingsoft, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación CodingSoft enumerada como una sesión separada.
...
Puede configurar codingsoft para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar codingsoft para usar modelos locales a través de LM Studio.
...
Puedes configurar codingsoft para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para CodingSoft. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/codingsoft) para obtener más detalles.
:::
...
Para usar Ollama Cloud con CodingSoft:
...
3. Copie la clave API para usarla en CodingSoft.
...
6. **Importante**: Antes de usar modelos en la nube en CodingSoft, debe obtener la información del modelo localmente:
...
CodingSoft Zen es una lista de modelos probados y verificados proporcionada por el equipo CodingSoft. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>CodingSoft Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **CodingSoft Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave CodingSoft API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración codingsoft.
...
5. También puedes personalizarlos a través de tu configuración codingsoft. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con CodingSoft:
...
También puede personalizar modelos a través de su configuración codingsoft. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con codingsoft. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `codingsoft.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a CodingSoft comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `codingsoft auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración codingsoft y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración codingsoft.
