...
Las herramientas permiten que LLM realice acciones en su código base. CodingSoft viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en CodingSoft.
...
:::note
Esta herramienta solo está disponible cuando `CODINGSOFT_EXPERIMENTAL_LSP_TOOL=true` (o `CODINGSOFT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor CodingSoft o cuando la variable de entorno `CODINGSOFT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar CodingSoft:

```bash
CODINGSOFT_ENABLE_EXA=1 codingsoft
```
