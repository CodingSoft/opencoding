...
description: Cree herramientas que LLM pueda llamar en codingsoft.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de codingsoft como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.codingsoft/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/codingsoft/tools/`.
...
```ts title=".codingsoft/tools/database.ts" {1}
import { tool } from "@codingsoft-ai/plugin"
...
```ts title=".codingsoft/tools/math.ts"
import { tool } from "@codingsoft-ai/plugin"
...
```ts title=".codingsoft/tools/bash.ts"
import { tool } from "@codingsoft-ai/plugin"
...
```ts title=".codingsoft/tools/project.ts" {8}
import { tool } from "@codingsoft-ai/plugin"
...
```python title=".codingsoft/tools/add.py"
import sys
...
```ts title=".codingsoft/tools/python-add.ts" {10}
import { tool } from "@codingsoft-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".codingsoft/tools/add.py")
