...
Las habilidades del agente permiten a CodingSoft descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
CodingSoft busca estas ubicaciones:

- Configuración del proyecto: `.codingsoft/skills/<name>/SKILL.md`
- Configuración global: `~/.config/codingsoft/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, CodingSoft sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.codingsoft/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/codingsoft/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.codingsoft/skills/git-release/SKILL.md` así:
...
CodingSoft enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `codingsoft.json`:
...
**Para agentes integrados** (en `codingsoft.json`):
