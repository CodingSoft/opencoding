---
title: Complementos
description: Escriba sus propios complementos para extender CodingSoft.
---

Los complementos le permiten extender CodingSoft al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de CodingSoft.
...
- `.codingsoft/plugins/` - Complementos a nivel de proyecto
- `~/.config/codingsoft/plugins/` - Complementos globales
...
```json title="codingsoft.json"
{
  ...
  "plugin": ["codingsoft-helicone-session", "codingsoft-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/codingsoft/node_modules/`.
...
1. Configuración global (`~/.config/codingsoft/codingsoft.json`)
2. Configuración del proyecto (`codingsoft.json`)
3. Directorio global de complementos (`~/.config/codingsoft/plugins/`)
4. Directorio de complementos del proyecto (`.codingsoft/plugins/`)
...
```json title=".codingsoft/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
CodingSoft ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".codingsoft/plugins/my-plugin.ts"
...
```js title=".codingsoft/plugins/example.js"
...
- `client`: Un cliente SDK codingsoft para interactuar con la IA.
...
import type { Plugin } from "@codingsoft-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar codingsoft.
...
```js title=".codingsoft/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "codingsoft"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio CodingSoft, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que codingsoft lea archivos `.env`:

```javascript title=".codingsoft/plugins/env-protection.js"
...
```javascript title=".codingsoft/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a codingsoft:

```ts title=".codingsoft/plugins/custom-tools.ts"
import { type Plugin, tool } from "@codingsoft-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que codingsoft puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para codingsoft junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://codingsoft.ai/docs/sdk) para obtener más detalles.
...
```ts title=".codingsoft/plugins/compaction.ts"
import type { Plugin } from "@codingsoft-ai/plugin"
...
```ts title=".codingsoft/plugins/custom-compaction.ts"
import type { Plugin } from "@codingsoft-ai/plugin"
