...
CodingSoft Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

:::note
CodingSoft Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en CodingSoft. Te suscribes a CodingSoft Go y
...
CodingSoft Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

CodingSoft Go funciona como cualquier otro proveedor en CodingSoft.

1. Inicias sesión en **<a href={console}>CodingSoft Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `CodingSoft Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a CodingSoft Go.
:::
...
CodingSoft Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de CodingSoft
usa el formato `codingsoft-go/<model-id>`. Por ejemplo, para Kimi K2.5, usarías
`codingsoft-go/kimi-k2.5` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con CodingSoft.
