---
title: Servidor
description: Interactuar con el servidor codingsoft a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `codingsoft serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente codingsoft puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `codingsoft.local` |
...
Configure `CODINGSOFT_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `codingsoft`, o configure `CODINGSOFT_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `codingsoft serve` como a `codingsoft web`.
...
Cuando ejecuta `codingsoft`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor codingsoft para interactuar con codingsoft mediante programación.
:::

Esta arquitectura permite que codingsoft admita múltiples clientes y le permite interactuar con codingsoft mediante programación.

Puede ejecutar `codingsoft serve` para iniciar un servidor independiente. Si tienes el
codingsoft TUI ejecutándose, `codingsoft serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos CodingSoft [IDE](/docs/ide).
...
El servidor codingsoft expone las siguientes API.
