---
title: Server
description: Interact with codingsoft server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `codingsoft serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an codingsoft client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `codingsoft.local` |
...
Set `CODINGSOFT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `codingsoft`, or set `CODINGSOFT_SERVER_USERNAME` to override it. This applies to both `codingsoft serve` and `codingsoft web`.
...
When you run `codingsoft` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the codingsoft server to interact with codingsoft programmatically.
:::

This architecture lets codingsoft support multiple clients and allows you to interact with codingsoft programmatically.

You can run `codingsoft serve` to start a standalone server. If you have the
codingsoft TUI running, `codingsoft serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CodingSoft [IDE](/docs/ide) plugins.
...
The codingsoft server exposes the following APIs.
