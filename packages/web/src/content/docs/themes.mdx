...
With CodingSoft you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, CodingSoft uses our own `codingsoft` theme.
...
CodingSoft comes with several built-in themes.
...
- Want CodingSoft to match their terminal's appearance
...
CodingSoft supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/codingsoft/themes/*.json` or `$XDG_CONFIG_HOME/codingsoft/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.codingsoft/themes/*.json`
4. **Current working directory** - Defined in `./.codingsoft/themes/*.json`
