...
W CodingSoft obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
CodingSoft zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
CodingSoft ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

CodingSoft ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `codingsoft.json`:
...
- Globalnie: `~/.config/codingsoft/agents/`
- W projekcie: `.codingsoft/agents/`
...
Jeśli nie zostanie skonfigurowana, CodingSoft używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji CodingSoft, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji CodingSoft w `provider/model-id`. Na przykład, [CodingSoft Zen](/docs/zen), użyj `codingsoft/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `codingsoft models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/codingsoft).
:::
