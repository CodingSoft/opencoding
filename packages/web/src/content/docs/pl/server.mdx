---
title: Serwer
description: Interakcja z serwerem codingsoft za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `codingsoft serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient codingsoft.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `codingsoft.local` |
...
Ustaw `CODINGSOFT_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `codingsoft` lub ustaw `CODINGSOFT_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `codingsoft serve`, jak i `codingsoft web`.
...
Kiedy uruchomisz `codingsoft`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera codingsoft do programowej interakcji z codingsoft.
:::

Ta architektura umożliwia obsługę wielu klientów przez codingsoft i programową interakcję z kodem otwartym.

Możesz uruchomić `codingsoft serve`, aby uruchomić samodzielny serwer. Jeśli masz
codingsoft TUI działa, `codingsoft serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki codingsoft [IDE](/docs/ide).
...
Serwer codingsoft udostępnia następujące interfejsy API.
