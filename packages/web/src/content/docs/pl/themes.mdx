...
Dzięki codingsoft możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie codingsoft używa naszego własnego motywu `codingsoft`.
...
codingsoft ma kilka wbudowanych motywów.
...
- Chcą, aby CodingSoft pasował do wyglądu ich terminala
...
codingsoft obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/codingsoft/themes/*.json` lub `$XDG_CONFIG_HOME/codingsoft/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.codingsoft/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.codingsoft/themes/*.json`
