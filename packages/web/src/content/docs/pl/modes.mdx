...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji codingsoft.
...
te lub skonfiguruj własne za pomocą konfiguracji codingsoft.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.codingsoft/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `codingsoft.json`:
...
- Globalnie: `~/.config/codingsoft/modes/`
...
Jeśli nie ma wpływu na temperaturę, codingsoft stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna codingsoft, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.codingsoft/modes/` dla trybów zapisanych dla projektu lub `~/.config/codingsoft/modes/` dla trybów globalnych:
