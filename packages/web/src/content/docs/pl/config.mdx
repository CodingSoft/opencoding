---
title: Konfiguracja
description: Ustawienia CodingSoft w pliku JSON.
---

Możesz dostosować CodingSoft za pomocą pliku konfiguracyjnego JSON.

---

## Format

CodingSoft obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "codingsoft"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/codingsoft`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/codingsoft/codingsoft.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `CODINGSOFT_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`codingsoft.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.codingsoft`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `CODINGSOFT_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.codingsoft` i `~/.config/codingsoft` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/codingsoft`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację CodingSoft w `~/.config/codingsoft/codingsoft.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/codingsoft/tui.json`.
...
Dodaj `codingsoft.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy CodingSoft się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `CODINGSOFT_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `CODINGSOFT_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.codingsoft` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.codingsoft`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`codingsoft.ai/config.json`**](https://codingsoft.ai/config.json).
...
Użyj `CODINGSOFT_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `codingsoft.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `codingsoft serve` i `codingsoft web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer CodingSoft.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `codingsoft.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie CodingSoft wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/codingsoft/agents/` lub `.codingsoft/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, CodingSoft powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`codingsoft run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/codingsoft/commands/` lub `.codingsoft/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
CodingSoft może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy CodingSoft nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie CodingSoft **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają CodingSoft o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.codingsoft/plugins/` lub `~/.config/codingsoft/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie CodingSoft tylko do określonych dostawców.
