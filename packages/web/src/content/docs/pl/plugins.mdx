---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć codingsoft.
---

Wtyczki umożliwiające wyłączenie codingsoft poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania codingsoft.
...
- `.codingsoft/plugins/` - Wtyczki na poziomie projektu
- `~/.config/codingsoft/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/codingsoft/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/codingsoft/codingsoft.json`)
2. Konfiguracja projektu (`codingsoft.json`)
3. Globalny katalog wtyczek (`~/.config/codingsoft/plugins/`)
4. Katalog wtyczek projektu (`.codingsoft/plugins/`)
...
codingsoft uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z codingsoft do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do codingsoft.
...
:::note
Jeśli korzystasz z aplikacji komputerowej codingsoft, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj codingsoft czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do codingsoft:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod codingsoft. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://codingsoft.ai/docs/sdk).
