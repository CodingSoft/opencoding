...
Możesz dodać dodatkowe informacje do codingsoft za pomocą _Model Context Protocol_, w skrócie MCP. codingsoft obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [codingsoft Config](https://codingsoft.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/codingsoft`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
codingsoft automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, codingsoft:
...
Jeśli serwer wymaga uwierzytelnienia, codingsoft poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `codingsoft mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji codingsoft bezpieczne przechowa tokeny w `~/.local/share/codingsoft/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w codingsoft, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie codingsoft, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć codingsoft z kontem Sentry.
