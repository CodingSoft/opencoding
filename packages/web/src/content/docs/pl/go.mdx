...
CodingSoft Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

:::note
CodingSoft Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w CodingSoft. Subskrybujesz CodingSoft Go i
...
CodingSoft Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

CodingSoft Go działa jak każdy inny dostawca w CodingSoft.

1. Logujesz się do **<a href={console}>CodingSoft Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `CodingSoft Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować CodingSoft Go.
:::
...
CodingSoft Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji CodingSoft
używa formatu `codingsoft-go/<model-id>`. Na przykład dla Kimi K2.5 należy użyć
`codingsoft-go/kimi-k2.5` w swojej konfiguracji.
...
Stworzyliśmy CodingSoft Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z CodingSoft.
