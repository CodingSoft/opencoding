---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w codingsoft.
---

import config from "../../../../config.mjs"
export const console = config.console

codingsoft używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji codingsoft.
...
w `~/.local/share/codingsoft/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim codingsoft
config.
...
CodingSoft Zen to lista modeli dostarczonych przez zespół codingsoft, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z codingsoft. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od CodingSoft Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `CodingSoft Zen` i przejdź do [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
Działa jak każdy inny dostawca w codingsoft i jest całkowicie opcjonalny w użyciu.

---

## CodingSoft Go

CodingSoft Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół codingsoft, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z codingsoft.

1. Uruchom polecenie `/connect` w TUI, wybierz `CodingSoft Go` i przejdź do [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
Aby używać Amazon Bedrock z codingsoft:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania codingsoft:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `codingsoft.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w codingsoft nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod codingsoft działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji codingsoft.
...
codingsoft używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować codingsoft
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`codingsoft.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [codingsoft-gitlab-auth](https://www.npmjs.com/package/codingsoft-gitlab-auth).

##### Konfiguracja

Customize through `codingsoft.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem codingsoft:
...
Aby używać Google Vertex AI z codingsoft:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania codingsoft.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą codingsoft, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [codingsoft-helicone-session](https://github.com/H2Shami/codingsoft-helicone-session), aby automatycznie rejestrować każdą konwersację codingsoft jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja codingsoft będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować codingsoft tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować codingsoft tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować codingsoft tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem codingsoft. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/codingsoft).
:::
...
Aby korzystać z Ollama Cloud z codingsoft:
...
3. Skopiuj klucz API do użycia w codingsoft.
...
6. **Ważne**: Przed użyciem modeli chmurowych w codingsoft musisz pobrać informacje o modelu lokalnie:
...
CodingSoft Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół codingsoft. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>CodingSoft Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **CodingSoft Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API codingsoft.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji codingsoft.
...
5. Możesz je także dostosować za pomocą konfiguracji codingsoft. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z codingsoft:
...
Możesz także dostosować modele za pomocą konfiguracji codingsoft. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z codingsoft. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `codingsoft.json` w katalogu projektu:
...
Pola `limit` pozwalają codingsoft zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `codingsoft auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację codingsoft i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji codingsoft.
