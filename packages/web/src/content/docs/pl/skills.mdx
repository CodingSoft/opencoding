...
Umiejętności agenta pozwalają codingsoft odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
codingsoft przeszukuje te lokalizacje:

- Project config: `.codingsoft/skills/<name>/SKILL.md`
- Global config: `~/.config/codingsoft/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu codingsoft przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.codingsoft/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/codingsoft/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.codingsoft/skills/git-release/SKILL.md` w ten sposób:
...
codingsoft wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `codingsoft.json`:
...
**Dla agentów wbudowanych** (w `codingsoft.json`):
