...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. CodingSoft posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w CodingSoft.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CODINGSOFT_EXPERIMENTAL_LSP_TOOL=true` (lub `CODINGSOFT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy CodingSoft lub gdy zmienna środowiskowa `CODINGSOFT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu CodingSoft:
