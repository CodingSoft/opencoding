...
CodingSoft Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
CodingSoft Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in CodingSoft. Du abonnierst CodingSoft Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
CodingSoft zu verwenden.
...
CodingSoft Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

CodingSoft Go funktioniert wie jeder andere Provider in CodingSoft.

1. Du meldest dich bei **<a href={console}>CodingSoft Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `CodingSoft Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann CodingSoft Go abonnieren.
:::
...
CodingSoft Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner CodingSoft Config verwendet das Format `codingsoft-go/<model-id>`. Für Kimi K2.5 würdest du beispielsweise `codingsoft-go/kimi-k2.5` in deiner Config verwenden.
...
Wir haben CodingSoft Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit CodingSoft zu nutzen.
