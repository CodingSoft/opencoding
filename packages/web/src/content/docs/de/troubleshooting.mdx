...
Wenn CodingSoft Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codingsoft\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `codingsoft --log-level DEBUG`.

---

## Speicher

codingsoft speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/codingsoft/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codingsoft`
...
CodingSoft Desktop startet im Hintergrund einen lokalen CodingSoft-Server (`codingsoft-cli`-Sidecar).
...
- Nur macOS: `CodingSoft`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/codingsoft/codingsoft.jsonc` (oder `~/.config/codingsoft/codingsoft.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/codingsoft/codingsoft.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codingsoft\codingsoft.jsonc`
...
CodingSoft kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/codingsoft/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codingsoft\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit CodingSoft ihn neu aufbauen kann.

1. Beende CodingSoft Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/codingsoft`
- **Linux**: loesche `~/.cache/codingsoft` (oder fuehre aus: `rm -rf ~/.cache/codingsoft`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\codingsoft`

3. Starte CodingSoft Desktop neu.

---

### Server-Verbindungsprobleme beheben

CodingSoft Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `codingsoft.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `CODINGSOFT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CODINGSOFT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt CodingSoft Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit CodingSoft-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

CodingSoft Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer CodingSoft in den OS-Einstellungen aktiviert sind, und
...
1. Beende CodingSoft Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von CodingSoft Desktop):

- `codingsoft.settings.dat` (desktop default server URL)
- `codingsoft.global.dat` und `codingsoft.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit CodingSoft hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `codingsoft models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\codingsoft`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. codingsoft installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\codingsoft`

2. Starte codingsoft neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt codingsoft, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
codingsoft erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
