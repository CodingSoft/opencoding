---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in CodingSoft.
---

import config from "../../../../config.mjs"
export const console = config.console

CodingSoft verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer CodingSoft-Konfiguration.
...
in `~/.local/share/codingsoft/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem CodingSoft anpassen
config.
...
CodingSoft Zen ist eine Liste von Modellen, die vom CodingSoft-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit CodingSoft funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit CodingSoft Zen zu beginnen.
:::
...
Es funktioniert wie jeder andere Anbieter in CodingSoft und ist völlig optional.

---

## CodingSoft Go

CodingSoft Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom CodingSoft-Team getestet und verifiziert wurden, dass sie gut mit CodingSoft funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `CodingSoft Go` und gehen Sie zu [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
So verwenden Sie Amazon Bedrock mit CodingSoft:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie CodingSoft ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `codingsoft.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in CodingSoft wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit CodingSoft ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre CodingSoft-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `CodingSoft`, Bereiche: `api`
...
um das Token im CodingSoft-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
CodingSoft verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von CodingSoft
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`codingsoft.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [codingsoft-gitlab-auth](https://www.npmjs.com/package/codingsoft-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `codingsoft.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit CodingSoft:
...
So verwenden Sie Google Vertex AI mit CodingSoft:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie CodingSoft ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über CodingSoft konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [codingsoft-helicone-session](https://github.com/H2Shami/codingsoft-helicone-session)-Plugin, um jede CodingSoft-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede CodingSoft-Konversation als separate Sitzung aufgeführt.
...
Sie können CodingSoft für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können CodingSoft über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können CodingSoft über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für CodingSoft konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/codingsoft).
:::
...
So verwenden Sie Ollama Cloud mit CodingSoft:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in CodingSoft.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in CodingSoft verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
CodingSoft Zen ist eine Liste getesteter und verifizierter Modelle, die vom CodingSoft-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>CodingSoft Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **CodingSoft Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren CodingSoft API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre CodingSoft-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre CodingSoft-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit CodingSoft:
...
Sie können Modelle auch über Ihre CodingSoft-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit CodingSoft verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `codingsoft.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann CodingSoft erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `codingsoft auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die CodingSoft-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer CodingSoft-Konfiguration übereinstimmt.
