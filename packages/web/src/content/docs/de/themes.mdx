...
Mit CodingSoft kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet CodingSoft das `codingsoft`-Theme.
...
CodingSoft liefert mehrere eingebaute Themes mit.
...
- CodingSoft optisch ans Terminal anpassen wollen
...
CodingSoft unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/codingsoft/themes/*.json` oder `$XDG_CONFIG_HOME/codingsoft/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.codingsoft/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.codingsoft/themes/*.json`
