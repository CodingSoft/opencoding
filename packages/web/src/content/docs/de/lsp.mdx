---
title: LSP Server
description: CodingSoft lässt sich in Ihren LSP-Server integrieren.
---

CodingSoft lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

CodingSoft verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `CODINGSOFT_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn CodingSoft eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer CodingSoft-Konfiguration anpassen.
...
```json title="codingsoft.json" {5-7}
{
  "$schema": "https://codingsoft.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="codingsoft.json" {5-9}
{
  "$schema": "https://codingsoft.ai/config.json",
  "lsp": {
    "typescript": {
