---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um CodingSoft zu erweitern.
---

Mit Plugins können Sie CodingSoft erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von CodingSoft zu ändern.
...
- `.codingsoft/plugins/` – Plugins auf Projektebene
- `~/.config/codingsoft/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/codingsoft/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/codingsoft/codingsoft.json`)
2. Projektkonfiguration (`codingsoft.json`)
3. Globales Plugin-Verzeichnis (`~/.config/codingsoft/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.codingsoft/plugins/`)
...
CodingSoft führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein CodingSoft-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie CodingSoft erweitern können.
...
:::note
Wenn Sie die CodingSoft-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass CodingSoft `.env`-Dateien liest:
...
Plugins können CodingSoft auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das CodingSoft aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für CodingSoft zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://codingsoft.ai/docs/sdk).
