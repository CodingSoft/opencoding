---
title: Server
description: Nutze den codingsoft-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `codingsoft serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein codingsoft-Client nutzen kann.
...
Setze `CODINGSOFT_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `codingsoft`, kann aber mit `CODINGSOFT_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `codingsoft serve` und `codingsoft web`.
...
Wenn du `codingsoft` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den codingsoft-Server, um codingsoft programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `codingsoft serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `codingsoft serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die CodingSoft-[IDE](/docs/ide)-Plugins.
...
Der codingsoft-Server stellt folgende APIs bereit.
