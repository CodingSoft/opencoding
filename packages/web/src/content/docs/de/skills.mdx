...
Agent Skills erlauben CodingSoft, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
CodingSoft sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft CodingSoft vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.codingsoft/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/codingsoft/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.codingsoft/skills/git-release/SKILL.md` zum Beispiel so:
...
CodingSoft listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `codingsoft.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `codingsoft.json`):
