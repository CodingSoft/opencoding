---
title: Regeln
description: Lege eigene Anweisungen fuer codingsoft fest.
---

Du kannst codingsoft ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in codingsoft den Befehl `/init` aus.
...
So kann codingsoft sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht codingsoft sie zu erweitern.
...
codingsoft kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/codingsoft/AGENTS.md` speichern.
Diese gelten in allen codingsoft-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt CodingSoft dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/codingsoft/AGENTS.md` existiert)
...
Beim Start sucht codingsoft in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/codingsoft/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `codingsoft.json` oder in `~/.config/codingsoft/codingsoft.json` angeben.
...
codingsoft parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit codingsoft.json

Empfohlen ist das `instructions`-Feld in der `codingsoft.json`:
...
Du kannst codingsoft explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `codingsoft.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
