...
Le skill degli agenti permettono a CodingSoft di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
CodingSoft cerca in queste posizioni:

- Config di progetto: `.codingsoft/skills/<name>/SKILL.md`
- Config globale: `~/.config/codingsoft/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, CodingSoft risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.codingsoft/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/codingsoft/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.codingsoft/skills/git-release/SKILL.md` cosi':
...
CodingSoft elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `codingsoft.json`:
...
**Per agenti integrati** (in `codingsoft.json`):
