---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di CodingSoft.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodingSoft fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire CodingSoft avvia la TUI per la directory corrente.
...
Quando usi la TUI di CodingSoft, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a CodingSoft. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da CodingSoft. _Alias_: `/quit`, `/q`
...
Questo è separato da `codingsoft.json`, che configura il comportamento di server/runtime.
...
Usa `CODINGSOFT_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
