...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di codingsoft. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in codingsoft ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di codingsoft.
...
codingsoft include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.codingsoft/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `codingsoft.json`:
...
- Globale: `~/.config/codingsoft/modes/`
- Progetto: `.codingsoft/modes/`
...
Se non viene specificata alcuna temperatura, codingsoft usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di codingsoft sia per quella specifica del progetto.
...
Crea file di modalita in `.codingsoft/modes/` per modalita specifiche del progetto oppure in `~/.config/codingsoft/modes/` per modalita globali:
