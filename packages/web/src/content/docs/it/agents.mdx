...
In CodingSoft esistono due tipi di agenti: agenti primari e subagenti.
...
CodingSoft include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
CodingSoft include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

CodingSoft include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `codingsoft.json`:
...
- Globale: `~/.config/codingsoft/agents/`
- Per progetto: `.codingsoft/agents/`
...
Se non specifichi una temperature, CodingSoft usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di CodingSoft sia per la config specifica del progetto.
...
L'ID modello nella configurazione CodingSoft usa il formato `provider/model-id`. Per esempio, se stai usando [CodingSoft Zen](/docs/zen), useresti `codingsoft/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `codingsoft models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/codingsoft).
:::
