...
Puoi aggiungere strumenti esterni a CodingSoft usando il _Model Context Protocol_, o MCP. CodingSoft supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [CodingSoft Config](https://codingsoft.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/codingsoft`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
CodingSoft gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, CodingSoft:
...
Se il server richiede autenticazione, CodingSoft ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `codingsoft mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, CodingSoft memorizzera' i token in modo sicuro in `~/.local/share/codingsoft/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in CodingSoft insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione CodingSoft come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare CodingSoft al tuo account Sentry.
