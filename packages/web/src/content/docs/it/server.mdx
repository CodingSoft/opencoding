---
title: Server
description: Interagisci con il server codingsoft via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `codingsoft serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client codingsoft.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `codingsoft.local` |
...
Imposta `CODINGSOFT_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `codingsoft`, oppure imposta `CODINGSOFT_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `codingsoft serve` sia per `codingsoft web`.
...
Quando esegui `codingsoft` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server codingsoft per interagire con codingsoft in modo programmatico.
:::

Questa architettura permette a codingsoft di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `codingsoft serve` per avviare un server standalone. Se la TUI di codingsoft e gia in esecuzione, `codingsoft serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di CodingSoft.
...
Il server codingsoft espone le seguenti API.
