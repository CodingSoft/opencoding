---
title: Plugin
description: Scrivi plugin per estendere CodingSoft.
---

I plugin ti permettono di estendere CodingSoft agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di CodingSoft.
...
- `.codingsoft/plugins/` - plugin a livello progetto
- `~/.config/codingsoft/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/codingsoft/node_modules/`.
...
1. Config globale (`~/.config/codingsoft/codingsoft.json`)
2. Config di progetto (`codingsoft.json`)
3. Directory plugin globale (`~/.config/codingsoft/plugins/`)
4. Directory plugin di progetto (`.codingsoft/plugins/`)
...
CodingSoft esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di codingsoft per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere codingsoft.
...
:::note
Se usi l'app desktop di CodingSoft, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a codingsoft di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a codingsoft:
...
L'helper `tool` crea uno strumento personalizzato che codingsoft può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in codingsoft insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://codingsoft.ai/docs/sdk) per i dettagli.
