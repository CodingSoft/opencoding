...
CodingSoft Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

:::note
CodingSoft Go is currently in beta.
:::

Go works like any other provider in CodingSoft. You subscribe to CodingSoft Go and
...
CodingSoft Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

CodingSoft Go works like any other provider in CodingSoft.

1. You sign in to **<a href={console}>CodingSoft Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `CodingSoft Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to CodingSoft Go.
:::
...
CodingSoft Go includes the following limits:
...
uses the format `codingsoft-go/<model-id>`. For example, for Kimi K2.5, you would
use `codingsoft-go/kimi-k2.5` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with CodingSoft as well.
