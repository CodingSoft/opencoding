---
title: TUI
description: Using the CodingSoft terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodingSoft provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running **CodingSoft** starts the TUI for the current directory.
...
When using the CodingSoft TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to CodingSoft. Allows you to select from available providers and add their API keys.
...
Exit CodingSoft. _Aliases_: `/quit`, `/q`
...
This is separate from `codingsoft.json`, which configures server/runtime behavior.
...
Use `CODINGSOFT_TUI_CONFIG` to load a custom TUI config path.
