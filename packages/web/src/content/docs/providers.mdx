---
title: Providers
description: Using any LLM provider in CodingSoft.
---

import config from "../../../config.mjs"
export const console = config.console

CodingSoft uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your CodingSoft config.
...
You can customize the providers through the `provider` section in your CodingSoft
config.
...
CodingSoft Zen is a list of models provided by the CodingSoft team that have been
tested and verified to work well with CodingSoft. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with CodingSoft Zen.
:::

1. Run the `/connect` command in the TUI, select `CodingSoft Zen`, and head to [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
It works like any other provider in CodingSoft and is completely optional to use.

---

## CodingSoft Go

CodingSoft Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the CodingSoft team that have been
tested and verified to work well with CodingSoft.

1. Run the `/connect` command in the TUI, select `CodingSoft Go`, and head to [codingsoft.ai/auth](https://codingsoft.ai/zen).
...
To use Amazon Bedrock with CodingSoft:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running codingsoft:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `codingsoft.json`:
...
CodingSoft. Anthropic explicitly prohibits this.

Previous versions of CodingSoft came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in CodingSoft with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for codingsoft to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your codingsoft config.
...
GitLab Duo support in CodingSoft is experimental. Features, configuration, and
behavior may change in future releases.
:::

CodingSoft integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
CodingSoft uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock CodingSoft
to only use your own GitLab-hosted instance, add the following to your
`codingsoft.json` file. It is also recommended to disable session sharing.
...
More documentation on [codingsoft-gitlab-auth](https://www.npmjs.com/package/codingsoft-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, CodingSoft will:
...
4. Route tool execution requests through CodingSoft's permission-gated tool system
...
To use your GitHub Copilot subscription with codingsoft:
...
To use Google Vertex AI with CodingSoft:
...
2. Set the required environment variables:
   ...
   Set them while running codingsoft.
...
In the event you see a feature or model from Helicone that isn't configured automatically through codingsoft, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [codingsoft-helicone-session](https://github.com/H2Shami/codingsoft-helicone-session) plugin to automatically log each CodingSoft conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each CodingSoft conversation listed as a separate session.
...
You can configure codingsoft to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure codingsoft to use local models through LM Studio.
...
You can configure codingsoft to use local models through Ollama.

:::tip
Ollama can automatically configure itself for CodingSoft. See the [Ollama integration docs](https://docs.ollama.com/integrations/codingsoft) for details.
:::
...
To use Ollama Cloud with CodingSoft:
...
3. Copy the API key for use in CodingSoft.
...
6. **Important**: Before using cloud models in CodingSoft, you must pull the model information locally:
...
CodingSoft Zen is a list of tested and verified models provided by the CodingSoft team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>CodingSoft Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **CodingSoft Zen**.

   ```txt
   /connect
   ```

3. Enter your CodingSoft API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your codingsoft config.
...
5. You can also customize them through your codingsoft config. Here's an example of specifying a provider
...
You can also customize models through your codingsoft config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with codingsoft. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `codingsoft.json` file in your project directory:
...
The `limit` fields allow CodingSoft to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `codingsoft auth list` to see if the credentials
...
2. For custom providers, check the codingsoft config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your codingsoft config.
