...
import { type Session } from "@codingsoft-ai/sdk/v2/client"
import {
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("codingsoft://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("codingsoft://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("codingsoft://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("codingsoft://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("codingsoft://open-project")).toBeUndefined()
    expect(parseDeepLink("codingsoft://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "codingsoft://open-project?directory=/a",
      "codingsoft://other?directory=/b",
      "codingsoft://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("codingsoft://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("codingsoft://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("codingsoft://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("codingsoft://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "codingsoft://new-session?directory=/a",
      "codingsoft://open-project?directory=/b",
      "codingsoft://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CODINGSOFT__: {
        deepLinks: ["codingsoft://open-project?directory=/a"],
      },
    } as unknown as Window & { __CODINGSOFT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["codingsoft://open-project?directory=/a"])
