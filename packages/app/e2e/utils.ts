...
import { base64Encode, checksum } from "@codingsoft-ai/util/encode"
...
export function createSdk(directory?: string) {
  return createOpencodingClient({ baseUrl: serverUrl, directory, throwOnError: true })
}
...
export function workspacePersistKey(directory: string, key: string) {
  ...
  return `codingsoft.workspace.${head}.${sum}.dat:workspace:${key}`
}
