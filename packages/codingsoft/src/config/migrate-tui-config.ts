...
const TUI_SCHEMA_URL = "https://codingsoft.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from codingsoft.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const codingsoft = await codingsoftFiles(input)
  for (const file of codingsoft) {
...
async function codingsoftFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.CODINGSOFT_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("codingsoft", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "codingsoft")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "codingsoft"))
  }
  if (Flag.CODINGSOFT_CONFIG) files.push(Flag.CODINGSOFT_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "codingsoft"))
