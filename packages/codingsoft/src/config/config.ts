...
import { NamedError } from "@codingsoft-ai/util/error"
...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/codingsoft"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "codingsoft")
      default:
        return "/etc/codingsoft"
    }
  }

  export function managedConfigDir() {
    return process.env.CODINGSOFT_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }
  ...
  export async function installDependencies(dir: string, input?: InstallInput) {
    ...
    await BunProc.run(
      ...
      if (err instanceof Process.RunFailedError) {
        ...
        if (Flag.CODINGSOFT_STRICT_CONFIG_DEPS) {
  ...
  export async function needsInstall(dir: string) {
    ...
    const mod = path.join(dir, "node_modules", "@codingsoft-ai", "plugin")
    ...
    const depVersion = dependencies["@codingsoft-ai/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      if (!online()) return false
      const stale = await PackageRegistry.isOutdated("@codingsoft-ai/plugin", depVersion, dir)
      if (!stale) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@codingsoft-ai/plugin",
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.codingsoft/command/", "/.codingsoft/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.codingsoft/agent/", "/.codingsoft/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
   ...
   * 2. Local codingsoft.json
   * 3. Global plugin/ directory
   * 4. Global codingsoft.json
   *
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: codingsoft.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for codingsoft serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://codingsoft.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://codingsoft.ai/docs/agents"),
      provider: z
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@codingsoft/Config") {}

  function globalConfigFile() {
    const candidates = ["codingsoft.jsonc", "codingsoft.json", "config.json"].map((file) =>
      ...
      Effect.gen(function* () {
        ...
        const loadConfig = Effect.fnUntraced(function* (
          ...
          const normalized = (() => {
            ...
            delete copy.tui
            log.warn("tui keys in codingsoft config are deprecated; move them to tui.json", { path: source })
            return copy
          })()
          ...
          if (parsed.success) {
            if (!parsed.data.$schema && isFile) {
              parsed.data.$schema = "https://codingsoft.ai/config.json"
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "codingsoft.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "codingsoft.jsonc"))),
          )
            ...
            yield* Effect.promise(() =>
              import(pathToFileURL(legacy).href, { with: { type: "toml" } })
                .then(async (mod) => {
                  ...
                  result["$schema"] = "https://codingsoft.ai/config.json"
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          for (const [key, value] of Object.entries(auth)) {
            if (value.type === "wellknown") {
              ...
              log.debug("fetching remote config", { url: `${url}/.well-known/codingsoft` })
              const response = yield* Effect.promise(() => fetch(`${url}/.well-known/codingsoft`))
              if (!response.ok) {
                throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
              }
              const wellknown = (yield* Effect.promise(() => response.json())) as any
              const remoteConfig = wellknown.config ?? {}
              if (!remoteConfig.$schema) remoteConfig.$schema = "https://codingsoft.ai/config.json"
              result = mergeConfigConcatArrays(
                result,
                yield* loadConfig(JSON.stringify(remoteConfig), {
                  dir: path.dirname(`${url}/.well-known/codingsoft`),
                  source: `${url}/.well-known/codingsoft`,
                }),
              )
          ...
          if (Flag.CODINGSOFT_CONFIG) {
            result = mergeConfigConcatArrays(result, yield* loadFile(Flag.CODINGSOFT_CONFIG))
            log.debug("loaded custom config", { path: Flag.CODINGSOFT_CONFIG })
          }

          if (!Flag.CODINGSOFT_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("codingsoft", ctx.directory, ctx.worktree),
            )) {
          ...
          if (Flag.CODINGSOFT_CONFIG_DIR) {
            log.debug("loading config from CODINGSOFT_CONFIG_DIR", { path: Flag.CODINGSOFT_CONFIG_DIR })
          }

          const deps: Promise<void>[] = []

          for (const dir of unique(directories)) {
            if (dir.endsWith(".codingsoft") || dir === Flag.CODINGSOFT_CONFIG_DIR) {
              for (const file of ["codingsoft.jsonc", "codingsoft.json"]) {
          ...
          if (process.env.CODINGSOFT_CONFIG_CONTENT) {
            result = mergeConfigConcatArrays(
              result,
              yield* loadConfig(process.env.CODINGSOFT_CONFIG_CONTENT, {
                dir: ctx.directory,
                source: "CODINGSOFT_CONFIG_CONTENT",
              }),
            )
            log.debug("loaded custom config from CODINGSOFT_CONFIG_CONTENT")
          }
          ...
          if (active?.active_org_id) {
            yield* Effect.gen(function* () {
              ...
              if (token) {
                process.env["CODINGSOFT_CONSOLE_TOKEN"] = token
                Env.set("CODINGSOFT_CONSOLE_TOKEN", token)
              }
          ...
          if (Flag.CODINGSOFT_PERMISSION) {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.CODINGSOFT_PERMISSION))
          }
          ...
          if (Flag.CODINGSOFT_DISABLE_AUTOCOMPACT) {
            result.compaction = { ...result.compaction, auto: false }
          }
          if (Flag.CODINGSOFT_DISABLE_PRUNE) {
