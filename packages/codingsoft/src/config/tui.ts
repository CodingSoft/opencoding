...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.CODINGSOFT_TUI_CONFIG
  }
  ...
  const state = Instance.state(async () => {
    let projectFiles = Flag.CODINGSOFT_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.CODINGSOFT_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".codingsoft") && dir !== Flag.CODINGSOFT_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old codingsoft.json shape) still get their settings applied.
