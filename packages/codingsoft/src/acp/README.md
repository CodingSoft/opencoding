...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for codingsoft.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal codingsoft sessions
...
codingsoft acp --cwd /path/to/project
```
...
```bash
CODINGSOFT_ENABLE_QUESTION_TOOL=1 codingsoft acp
```
...
- **Terminal Integration**: Full terminal support via codingsoft's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | codingsoft acp
```
...
### Mapping to CodingSoft

ACP sessions map cleanly to codingsoft's internal session model:
