...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@codingsoft-ai/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `codingsoft auth login` in the terminal",
        name: "Login with codingsoft",
        id: "codingsoft-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // CodingSoft stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const codingsoftProvider = providers.find((p) => p.id === "codingsoft")
    if (codingsoftProvider) {
      if (codingsoftProvider.models["big-pickle"]) {
        return { providerID: ProviderID.codingsoft, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(codingsoftProvider.models))
      if (best) {
        return {
