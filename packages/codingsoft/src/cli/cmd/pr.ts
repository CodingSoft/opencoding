...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run codingsoft",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for codingsoft session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found codingsoft session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["codingsoft", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting codingsoft...")
        UI.println()

        const codingsoftArgs = sessionId ? ["-s", sessionId] : []
        const codingsoftProcess = Process.spawn(["codingsoft", ...codingsoftArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await codingsoftProcess.exited
        if (code !== 0) throw new Error(`codingsoft exited with code ${code}`)
      },
    })
  },
})
