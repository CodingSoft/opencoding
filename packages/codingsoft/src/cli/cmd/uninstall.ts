...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall codingsoft and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall CodingSoft")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g codingsoft-ai",
      pnpm: "pnpm uninstall -g codingsoft-ai",
      bun: "bun remove -g codingsoft-ai",
      yarn: "yarn global remove codingsoft-ai",
      brew: "brew uninstall codingsoft",
      choco: "choco uninstall codingsoft",
      scoop: "scoop uninstall codingsoft",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "codingsoft", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using CodingSoft!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# codingsoft") || content.includes(".codingsoft/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".codingsoft/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".codingsoft/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".codingsoft"))
    ) {
      continue
    }
