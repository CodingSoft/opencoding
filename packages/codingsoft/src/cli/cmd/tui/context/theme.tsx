...
import codingsoft from "./theme/codingsoft.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@codingsoft-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "codingsoft")
        draft.active = typeof active === "string" ? active : "codingsoft"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.codingsoft, store.mode)
    })
