...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".codingsoft"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".codingsoft", "themes")
...
function defaultPluginRecord(state: RuntimeState, spec: string): TuiConfig.PluginRecord {
  return {
    item: spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".codingsoft", "tui.json"),
  }
}
  ...
  async function load(api: Api) {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const records = Flag.CODINGSOFT_PURE ? [] : (config.plugin_records ?? [])
        if (Flag.CODINGSOFT_PURE && config.plugin_records?.length) {
