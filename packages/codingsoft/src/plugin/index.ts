import type { Hooks, PluginInput, Plugin as PluginInstance, PluginModule } from "@codingsoft-ai/plugin"
import { Config } from "../config/config"
import { Bus } from "../bus"
import { Log } from "../util/log"
import { createOpencodingClient } from "@codingsoft-ai/sdk"
import { Flag } from "../flag/flag"
import { CodexAuthPlugin } from "./codex"
import { Session } from "../session"
import { NamedError } from "@codingsoft-ai/util/error"
import { CopilotAuthPlugin } from "./copilot"
import { gitlabAuthPlugin as GitlabAuthPlugin } from "codingsoft-gitlab-auth"
import { PoeAuthPlugin } from "codingsoft-poe-auth"
...
export namespace Plugin {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@codingsoft/Plugin") {}
      ...
      const cache = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createOpencodingClient({
            ...
            headers: Flag.CODINGSOFT_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.CODINGSOFT_SERVER_USERNAME ?? "codingsoft"}:${Flag.CODINGSOFT_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.CODINGSOFT_PURE ? [] : (cfg.plugin ?? [])
          if (Flag.CODINGSOFT_PURE && cfg.plugin?.length) {
