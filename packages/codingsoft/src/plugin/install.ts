...
export type PatchDeps = {
  ...
  files: (dir: string, name: "codingsoft" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".codingsoft")
}

function patchName(kind: Kind): "codingsoft" | "tui" {
  if (kind === "server") return "codingsoft"
  return "tui"
}
