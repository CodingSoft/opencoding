...
export const DEPRECATED_PLUGIN_PACKAGES = ["codingsoft-openai-codex-auth", "codingsoft-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, codingsoftVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(codingsoftVersion) || semver.major(codingsoftVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.codingsoft
  if (typeof range !== "string") return
  if (!semver.satisfies(codingsoftVersion, range)) {
    throw new Error(`Plugin requires codingsoft ${range} but running ${codingsoftVersion}`)
  }
}
