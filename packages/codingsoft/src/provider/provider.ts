...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async codingsoft(input) {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["codingsoft"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from codingsoft.json provider config
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://codingsoft.ai/",
            "X-Title": "codingsoft",
          },
        },
      }
    ...
    gitlab: async (input) => {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `codingsoft/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `codingsoft auth cloudflare-ai-gateway`.",
        )
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "codingsoft",
          },
        },
      }
    },
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@codingsoft/Provider") {}
  ...
  const layer: Layer.Layer<Service, never, Config.Service | Auth.Service> = Layer.effect(
    ...
    Effect.gen(function* () {
        ...
        Effect.gen(function* () {
          ...
          for (const [id, provider] of Object.entries(providers)) {
            ...
            for (const [modelID, model] of Object.entries(provider.models)) {
              ...
              if (model.status === "alpha" && !Flag.CODINGSOFT_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
      ...
      const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
        ...
        if (providerID.startsWith("codingsoft")) {
          priority = ["gpt-5-nano"]
        }
