...
const FILES = [
  "AGENTS.md",
  ...(Flag.CODINGSOFT_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.CODINGSOFT_CONFIG_DIR) {
    files.push(path.join(Flag.CODINGSOFT_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.CODINGSOFT_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.CODINGSOFT_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no CODINGSOFT_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.CODINGSOFT_CONFIG_DIR, Flag.CODINGSOFT_CONFIG_DIR).catch(() => [])
}
