import { Slug } from "@codingsoft-ai/util/slug"
...
export namespace Session {
  ...
  export function plan(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".codingsoft", "plans")
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@codingsoft/Session") {}
  ...
  export const layer: Layer.Layer<Service, never, Bus.Service | Config.Service> = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const createNext = Effect.fn("Session.createNext")(function* (input: {
        ...
        if (!result.parentID && (Flag.CODINGSOFT_AUTO_SHARE || cfg.share === "auto")) {
          yield* share(result.id).pipe(Effect.ignore, Effect.forkIn(scope))
        }

        if (!Flag.CODINGSOFT_EXPERIMENTAL_WORKSPACES) {
