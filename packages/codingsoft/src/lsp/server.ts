...
import { Module } from "@codingsoft-ai/util/module"
...
export namespace LSPServer {
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        if (!(await Filesystem.exists(js))) {
          if (Flag.CODINGSOFT_DISABLE_LSP_DOWNLOAD) return
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "codingsoft-jdtls-data"))
      return {
        process: spawn(
          java,
          [
