  ...
  export const ControlPlaneRoutes = (opts?: { cors?: string[] }): Hono => {
    ...
    return app
      ...
      .use((c, next) => {
        ...
        const password = Flag.CODINGSOFT_SERVER_PASSWORD
        if (!password) return next()
        const username = Flag.CODINGSOFT_SERVER_USERNAME ?? "codingsoft"
      ...
      .use(
        cors({
          ...
          origin(input) {
            ...
            // *.codingsoft.ai (https only, adjust if needed)
            if (/^https:\/\/([a-z0-9-]+\.)*codingsoft\.ai$/.test(input)) {
              return input
            }
