...
const managedConfigDir = process.env.CODINGSOFT_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "codingsoft.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "codingsoft.json") {
...
test("ignores legacy tui keys in codingsoft config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://codingsoft.ai/config.json",
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "codingsoft.jsonc"),
        `{
        // This is a comment
        "$schema": "https://codingsoft.ai/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "codingsoft.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["CODINGSOFT_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { codingsoft: { options: { apiKey: "{env:CODINGSOFT_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["codingsoft"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["CODINGSOFT_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["CODINGSOFT_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "codingsoft.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .codingsoft directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codingsoftDir = path.join(dir, ".codingsoft")
      await fs.mkdir(codingsoftDir, { recursive: true })
      const agentDir = path.join(codingsoftDir, "agent")
...
test("loads agents from .codingsoft/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codingsoftDir = path.join(dir, ".codingsoft")
      await fs.mkdir(codingsoftDir, { recursive: true })

      const agentsDir = path.join(codingsoftDir, "agents")
...
test("loads commands from .codingsoft/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codingsoftDir = path.join(dir, ".codingsoft")
      await fs.mkdir(codingsoftDir, { recursive: true })

      const commandDir = path.join(codingsoftDir, "command")
...
test("loads commands from .codingsoft/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codingsoftDir = path.join(dir, ".codingsoft")
      await fs.mkdir(codingsoftDir, { recursive: true })

      const commandsDir = path.join(codingsoftDir, "commands")
...
test("does not try to install dependencies in read-only CODINGSOFT_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.CODINGSOFT_CONFIG_DIR
  process.env.CODINGSOFT_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.CODINGSOFT_CONFIG_DIR
    else process.env.CODINGSOFT_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable CODINGSOFT_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
  ...
  const run = spyOn(BunProc, "run").mockImplementation(async (_cmd, opts) => {
    const mod = path.join(opts?.cwd ?? "", "node_modules", "@codingsoft-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@codingsoft-ai/plugin", version: "1.0.0" }),
    )
    return {
      code: 0,
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "codingsoft.json"),
        JSON.stringify({ $schema: "https://codingsoft.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .codingsoft config
      const projectDir = path.join(dir, "project")
      const codingsoftDir = path.join(projectDir, ".codingsoft")
      await fs.mkdir(codingsoftDir, { recursive: true })
      ...
      // Local .codingsoft config with different plugins
      await Filesystem.write(
        path.join(codingsoftDir, "codingsoft.json"),
        JSON.stringify({
          $schema: "https://codingsoft.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .codingsoft config with some overlapping plugins
      await Filesystem.write(
        path.join(codingsoftDir, "codingsoft.json"),
        JSON.stringify({
          $schema: "https://codingsoft.ai/config.json",
...
// Note: preload.ts sets CODINGSOFT_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .codingsoft config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .codingsoft directory config enables it
      const codingsoftDir = path.join(dir, ".codingsoft")
      await fs.mkdir(codingsoftDir, { recursive: true })
      await Filesystem.write(
        path.join(codingsoftDir, "codingsoft.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/codingsoft")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/codingsoft")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "codingsoft.json")
    expect(await Config.resolvePluginSpec("oh-my-codingsoft@2.4.3", file)).toBe("oh-my-codingsoft@2.4.3")
...
describe("deduplicatePlugins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-codingsoft@2.4.3", "file:///project/.codingsoft/plugin/oh-my-codingsoft.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.codingsoft/plugin/demo.ts", "file:///project/.codingsoft/plugin/demo.ts"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result).toEqual(["file:///project/.codingsoft/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const codingsoftDir = path.join(projectDir, ".codingsoft")
        const pluginDir = path.join(codingsoftDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("CODINGSOFT_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"]
    process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .codingsoft/ directories when flag is set", async () => {
    const originalEnv = process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"]
    process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .codingsoft directory with a command
          const codingsoftDir = path.join(dir, ".codingsoft", "command")
          await fs.mkdir(codingsoftDir, { recursive: true })
          await Filesystem.write(path.join(codingsoftDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .codingsoft should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CODINGSOFT_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["CODINGSOFT_CONFIG_DIR"]
      process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["CODINGSOFT_CONFIG_DIR"]
      } else {
        process.env["CODINGSOFT_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("CODINGSOFT_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CODINGSOFT_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CODINGSOFT_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CODINGSOFT_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from CODINGSOFT_CONFIG_DIR, not project
...
describe("CODINGSOFT_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in CODINGSOFT_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CODINGSOFT_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["CODINGSOFT_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://codingsoft.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["CODINGSOFT_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["CODINGSOFT_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in CODINGSOFT_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CODINGSOFT_CONFIG_CONTENT"]
