...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const codingsoftModel = {
      ...openaiModel,
      providerID: "codingsoft",
      api: {
        id: "codingsoft-test",
        url: "https://api.codingsoft.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, codingsoftModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.codingsoft?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.codingsoft?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          codingsoft: { itemId: "msg_codingsoft" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              codingsoft: { itemId: "msg_codingsoft_part" },
    ...
    expect(result[0].providerOptions?.codingsoft?.itemId).toBe("msg_codingsoft")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.codingsoft?.itemId).toBe("msg_codingsoft_part")
