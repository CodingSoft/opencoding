...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CODINGSOFT_TEST ?? "")'), {
      env: {
        CODINGSOFT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CODINGSOFT_TEST_SHELL"], {
      shell: true,
      env: {
        CODINGSOFT_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CODINGSOFT_TEST_SHELL=ok")
  })
