...
Replace runtime Bun shell template-tag usage in `packages/codingsoft/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/codingsoft/src` is removed except:
