...
- Author package entrypoint is `@codingsoft-ai/plugin/tui`.
...
```json
{
  ...
  "theme": "smoke-theme",
  "plugin": ["@acme/codingsoft-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@codingsoft-ai/plugin/tui`.
- `@codingsoft-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@codingsoft-ai/plugin/tui"
...
- The value is a semver range checked against the running CodingSoft version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.codingsoft` is absent, no check is performed (backward compatible).
...
- `codingsoft plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `codingsoft plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.codingsoft` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.codingsoft`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.codingsoft`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.codingsoft` dir or `CODINGSOFT_CONFIG_DIR`, CodingSoft installs `@codingsoft-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@codingsoft-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.codingsoft/themes` area near the plugin config source.
...
- `--pure` / `CODINGSOFT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.codingsoft/plugins/tui-smoke.tsx`
- Local smoke config: `.codingsoft/tui.json`
- Local smoke theme: `.codingsoft/plugins/smoke-theme.json`
