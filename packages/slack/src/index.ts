...
import { createOpencoding, type ToolPart } from "@codingsoft-ai/sdk"
...
const codingsoft = await createOpencoding({
  port: 0,
})
...
;(async () => {
  const events = await codingsoft.client.event.subscribe()
