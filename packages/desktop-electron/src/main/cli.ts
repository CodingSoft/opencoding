...
const CLI_INSTALL_DIR = ".codingsoft/bin"
const CLI_BINARY_NAME = "codingsoft"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `codingsoft-cli${suffix}`)
    : join(root, "../../resources", `codingsoft-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "codingsoft-install.sh")
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    CODINGSOFT_SERVER_USERNAME: "codingsoft",
    CODINGSOFT_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    CODINGSOFT_EXPERIMENTAL_ICON_DISCOVERY: "true",
    CODINGSOFT_EXPERIMENTAL_FILEWATCHER: "true",
    CODINGSOFT_CLIENT: "desktop",
...
function buildCommand(args: string, env: Record<string, string>) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.codingsoft/bin/codingsoft"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://codingsoft.ai/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",
