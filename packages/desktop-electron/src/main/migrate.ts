...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.codingsoft.desktop.dev",
  beta: "ai.codingsoft.desktop.beta",
  prod: "ai.codingsoft.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.codingsoft.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `codingsoft.settings.dat` is special: it maps to the `codingsoft.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"codingsoft.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // codingsoft.settings.dat → the electron settings store ("codingsoft.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "codingsoft.global.dat").
  const storeName = filename === "codingsoft.settings.dat" ? "codingsoft.settings" : filename
