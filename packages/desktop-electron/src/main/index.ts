...
app.setPath("userData", join(app.getPath("appData"), app.isPackaged ? APP_IDS[CHANNEL] : "ai.codingsoft.desktop.dev"))
...
function setupApp() {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("codingsoft://"))
    if (urls.length) {
  ...
  void app.whenReady().then(async () => {
    // migrate()
    app.setAsDefaultProtocolClient("codingsoft")
    setDockIcon()
    setupAutoUpdater()
    syncCli()
    await initialize()
...
async function getSidecarPort() {
  const fromEnv = process.env.CODINGSOFT_PORT
  if (fromEnv) {
...
function sqliteFileExists() {
  ...
  return existsSync(join(base, "codingsoft", "codingsoft.db"))
}
