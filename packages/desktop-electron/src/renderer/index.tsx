...
import "./styles.css"
import { useTheme } from "@codingsoft-ai/ui/theme"
...
const deepLinkEvent = "codingsoft:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__CODINGSOFT__ ??= {}
  const pending = window.__CODINGSOFT__.deepLinks ?? []
  window.__CODINGSOFT__.deepLinks = [...pending, ...urls]
  window.dispatchEvent(new CustomEvent(deepLinkEvent, { detail: { urls } }))
}

const listenForDeepLinks = () => {
  const startUrls = window.__CODINGSOFT__?.deepLinks ?? []
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__CODINGSOFT__?.wsl) return undefined
    return window.api.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__CODINGSOFT__?.wsl) return result
  ...
  return {
    ...
    getWslEnabled: async () => {
      ...
      return window.__CODINGSOFT__!.wsl ?? false
    },
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("codingsoft.global.dat").getItem("language")
