...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("codingsoft-find")
  api.delete("codingsoft-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("codingsoft-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("codingsoft-find", new Highlight(...rest))
    return true
  }
